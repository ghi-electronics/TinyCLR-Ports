...
IF "%DoBuild%" == "1" (
    ...
    FOR %%A IN ("%ScriptRoot%\Targets\%ProcessorPart%", "%ScriptRoot%\Main") DO (
        ...
        FOR /R %%B IN ("*.cpp") DO (
            ...
            "%GccDirectory%\bin\arm-none-eabi-g++.exe" -std=c++11 -xc++ -w -Wabi %AdditionalCompilerArguments% -mcpu=%MCpu% -mlittle-endian %FloatCompileArguments% %AdditionalDefines% %AdditionalIncludes% -o "%OutputDirectory%\%%~nB.obj" -c "%%B"
        )
