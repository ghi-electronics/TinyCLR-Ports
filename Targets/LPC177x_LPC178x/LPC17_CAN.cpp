...
void LPC17_Can_EventCallback(const TinyCLR_Task_Manager* self, const TinyCLR_Api_Manager* apiManager, TinyCLR_Task_Reference task, void* arg);
bool LPC17_Can_CanPostEvent(int8_t controllerIndex);
...
void LPC17_Can_AddApi(const TinyCLR_Api_Manager* apiManager) {
    for (int32_t i = 0; i < TOTAL_CAN_CONTROLLERS; i++) {
        ...
        canControllers[i].GetMessagesToRead = &LPC17_Can_GetMessagesToRead;
        canControllers[i].GetMessagesToWrite = &LPC17_Can_GetMessagesToWrite;
        canControllers[i].SetMessageReceivedHandler = &LPC17_Can_SetMessageReceivedHandler;
        canControllers[i].SetErrorReceivedHandler = &LPC17_Can_SetErrorReceivedHandler;
        ...
        canControllers[i].GetReadBufferSize = &LPC17_Can_GetReadBufferSize;
        canControllers[i].SetReadBufferSize = &LPC17_Can_SetReadBufferSize;
        canControllers[i].GetWriteBufferSize = &LPC17_Can_GetWriteBufferSize;
...
bool LPC17_Can_ErrorHandler(uint8_t controllerIndex) {
    auto state = &canStates[controllerIndex];
...
void CAN_ISR_Rx(int32_t controllerIndex) {
    ...
    if (state->canDataFilter.groupFiltersSize || state->canDataFilter.matchFiltersSize) {
        uint32_t ID = controllerIndex == 0 ? C1RID : C2RID;

        char passed = 0;

        if (state->canDataFilter.groupFiltersSize) {
            if (BinarySearch2(state->canDataFilter.lowerBoundFilters, state->canDataFilter.upperBoundFilters, 0, state->canDataFilter.groupFiltersSize - 1, ID) >= 0)
                passed = 1;
        }

        if (!passed && state->canDataFilter.matchFiltersSize) {
            if (BinarySearch(state->canDataFilter.matchFilters, 0, state->canDataFilter.matchFiltersSize - 1, ID) >= 0)
                passed = 1;
        }
    ...
    // timestamp
    t = LPC17_Time_GetSystemTime(nullptr);
...
TinyCLR_Can_Error LPC17_Can_GetError(uint32_t error) {
    switch (error) {
    case 1:
...
void LPC17_Can_EventCallback(const TinyCLR_Task_Manager* self, const TinyCLR_Api_Manager* apiManager, TinyCLR_Task_Reference task, void* arg) {
    ...
    if (task == state->messageReceivedCallbackTaskReference) {
        ...
        if (latestCount > 0 && state->messageReceivedEventHandler != nullptr) {
            state->messageReceivedEventHandler(state->controller, latestCount, LPC17_Time_GetSystemTime(nullptr));
        }

        state->taskManager->Enqueue(state->taskManager, task, LPC17_Time_GetProcessorTicksForTime(nullptr, CAN_EVENT_POST_DEBOUNCE_TICKS));
    }
        ...
        if ((latestError != 0) && state->errorEventHandler != nullptr) {
            state->errorEventHandler(state->controller, LPC17_Can_GetError(latestError), LPC17_Time_GetSystemTime(nullptr));
        }
        state->taskManager->Enqueue(state->taskManager, task, LPC17_Time_GetProcessorTicksForTime(nullptr, CAN_EVENT_POST_DEBOUNCE_TICKS));
    }
}
...
TinyCLR_Result LPC17_Can_SetMessageReceivedHandler(const TinyCLR_Can_Controller* self, TinyCLR_Can_MessageReceivedHandler handler) {
    ...
    if (handler != nullptr) {
        ...
        state->taskManager->Create(state->taskManager, LPC17_Can_EventCallback, (void*)state, false, state->messageReceivedCallbackTaskReference);
        state->taskManager->Enqueue(state->taskManager, state->messageReceivedCallbackTaskReference, LPC17_Time_GetProcessorTicksForTime(nullptr, CAN_EVENT_POST_DEBOUNCE_TICKS));
    }
...
TinyCLR_Result LPC17_Can_SetErrorReceivedHandler(const TinyCLR_Can_Controller* self, TinyCLR_Can_ErrorReceivedHandler handler) {
    ...
    if (handler != nullptr) {
        ...
        state->taskManager = (const TinyCLR_Task_Manager*)apiManager->FindDefault(apiManager, TinyCLR_Api_Type::TaskManager);
        state->taskManager->Create(state->taskManager, LPC17_Can_EventCallback, (void*)state, false, state->errorCallbackTaskReference);
        state->taskManager->Enqueue(state->taskManager, state->errorCallbackTaskReference, LPC17_Time_GetProcessorTicksForTime(nullptr, CAN_EVENT_POST_DEBOUNCE_TICKS));
    }
    else {
...
TinyCLR_Result LPC17_Can_SetReadBufferSize(const TinyCLR_Can_Controller* self, size_t size) {
    ...
    state->canRxMessagesFifo = (LPC17_Can_Message*)memoryProvider->Allocate(memoryProvider, state->rxBufferSize * sizeof(LPC17_Can_Message));
