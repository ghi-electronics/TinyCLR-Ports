...
#define MCIClock (*(volatile unsigned long *)0x400C0004)
...
#define MCIArgument (*(volatile unsigned long *)0x400C0008)
#define MCIArgument_OFFSET 0x8

#define MCICommand (*(volatile unsigned long *)0x400C000C)
...
#define MCIRespCmd (*(volatile unsigned long *)0x400C0010)
#define MCIRespCmd_OFFSET 0x10
#define MCIRespCmd_RespCmd_MASK 0x3F
#define MCIRespCmd_RespCmd_BIT 0

#define MCIResponse20 (*(volatile unsigned long *)0x400C0014)
#define MCIResponse20_OFFSET 0x14

#define MCIResponse24 (*(volatile unsigned long *)0x400C0014)
#define MCIResponse24_OFFSET 0x14

#define MCIResponse28 (*(volatile unsigned long *)0x400C0014)
#define MCIResponse28_OFFSET 0x14

#define MCIResponse32 (*(volatile unsigned long *)0x400C0014)
#define MCIResponse32_OFFSET 0x14

#define MCIDataTimer (*(volatile unsigned long *)0x400C0024)
#define MCIDataTimer_OFFSET 0x24

#define MCIDataLength (*(volatile unsigned long *)0x400C0028)
#define MCIDataLength_OFFSET 0x28
#define MCIDataLength_DataLength_MASK 0xFFFF
#define MCIDataLength_DataLength_BIT 0

#define MCIDataCtrl (*(volatile unsigned long *)0x400C002C)
...
#define MCIDataCnt (*(volatile unsigned long *)0x400C0030)
#define MCIDataCnt_OFFSET 0x30
#define MCIDataCnt_DataCount_MASK 0xFFFF
#define MCIDataCnt_DataCount_BIT 0

#define MCIStatus (*(volatile unsigned long *)0x400C0034)
...
#define MCIClear (*(volatile unsigned long *)0x400C0038)
...
#define MCIMask0 (*(volatile unsigned long *)0x400C003C)
#define MCIMask0_OFFSET 0x3C

#define MCIMask1 (*(volatile unsigned long *)0x400C0040)
#define MCIMask1_OFFSET 0x40

#define MCIFifoCnt (*(volatile unsigned long *)0x400C0048)
#define MCIFifoCnt_OFFSET 0x48
#define MCIFifoCnt_DataCount_MASK 0x7FFF
#define MCIFifoCnt_DataCount_BIT 0

#define MCIFIFO (*(volatile unsigned long *)0x400C0080)
...
#define MCI_CLK_375KHZ            79
#define MCI_CLK_10MHz_DEFAULT    2
...
void MCI_Init(void) {
    ...
    (*(volatile unsigned long *)0x400FC1A0) |= 0x08;
...
bool MCI_And_Card_initialize(void) {
    ...
    if (!err) {
        MCI_Set_MCIClock(MCI_CLK_10MHz_DEFAULT);
...
bool MCI_WriteSector(
    ...
    while (i < WRITE_TIME_OUT) {
        ...
        i++;
        LPC17_Time_Delay(nullptr, 1000); // WRITE_TIME_OUT unit is ms = 1000*x;
    }
...
static const LPC17_Gpio_Pin sdCardData1Pins[] = LPC17_SD_DATA1_PINS;
static const LPC17_Gpio_Pin sdCardData2Pins[] = LPC17_SD_DATA2_PINS;
static const LPC17_Gpio_Pin sdCardData3Pins[] = LPC17_SD_DATA3_PINS;
static const LPC17_Gpio_Pin sdCardClkPins[] = LPC17_SD_CLK_PINS;
static const LPC17_Gpio_Pin sdCardCmdPins[] = LPC17_SD_CMD_PINS;
...
const TinyCLR_Api_Info* LPC17_SdCard_GetApi() {
    for (auto i = 0; i < TOTAL_SDCARD_CONTROLLERS; i++) {
        ...
        sdCardControllers[i].Release = &LPC17_SdCard_Release;

        sdCardControllers[i].WriteSectors = &LPC17_SdCard_WriteSectors;
        sdCardControllers[i].ReadSectors = &LPC17_SdCard_ReadSectors;
        sdCardControllers[i].EraseSectors = &LPC17_SdCard_EraseSectors;
        sdCardControllers[i].IsSectorErased = &LPC17_SdCard_IsSectorErased;
        sdCardControllers[i].GetSectorMap = &LPC17_SdCard_GetSectorMap;

        sdCardApi[i].Author = "GHI Electronics, LLC";
        sdCardApi[i].Name = "GHIElectronics.TinyCLR.NativeApis.LPC17.SdCardController";
...
TinyCLR_Result LPC17_SdCard_WriteSectors(const TinyCLR_SdCard_Controller* self, uint64_t sector, size_t& count, const uint8_t* data, uint32_t timeout) {
    int32_t index = 0;
...
TinyCLR_Result LPC17_SdCard_ReadSectors(const TinyCLR_SdCard_Controller* self, uint64_t sector, size_t& count, uint8_t* data, uint32_t timeout) {
    int32_t index = 0;
...
TinyCLR_Result LPC17_SdCard_EraseSectors(const TinyCLR_SdCard_Controller* self, uint64_t sector, size_t& count, uint32_t timeout) {
...
TinyCLR_Result LPC17_SdCard_GetSectorMap(const TinyCLR_SdCard_Controller* self, const size_t*& sizes, size_t& count, bool& isUniform) {
    ...
    state->sectorSizes[0] = LPC17_SD_SECTOR_SIZE;

    sizes = state->sectorSizes;
    count = sdMediaSize / LPC17_SD_SECTOR_SIZE;
