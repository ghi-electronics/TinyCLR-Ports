...
void STM32F4_Startup_GetDebuggerTransportProvider(const TinyCLR_Api_Info*& api, size_t& index, const void*& configuration) {
    ...
    provider->AcquirePin(provider, STM32F4_GpioInternal_GetControllerId(), DEBUGGER_SELECTOR_PIN);
    provider->SetDriveMode(provider, STM32F4_GpioInternal_GetControllerId(), DEBUGGER_SELECTOR_PIN, DEBUGGER_SELECTOR_PULL);
    provider->Read(provider, STM32F4_GpioInternal_GetControllerId(), DEBUGGER_SELECTOR_PIN, value);
    provider->ReleasePin(provider, STM32F4_GpioInternal_GetControllerId(), DEBUGGER_SELECTOR_PIN);
...
void STM32F4_Startup_GetRunApp(bool& runApp) {
    ...
    TinyCLR_Gpio_PinValue value;
    auto provider = static_cast<const TinyCLR_Gpio_Provider*>(STM32F4_Gpio_GetApi()->Implementation);
    provider->AcquirePin(provider, STM32F4_GpioInternal_GetControllerId(), RUN_APP_PIN);
    provider->SetDriveMode(provider, STM32F4_GpioInternal_GetControllerId(), RUN_APP_PIN, RUN_APP_PULL);
    provider->Read(provider, STM32F4_GpioInternal_GetControllerId(), RUN_APP_PIN, value);
    provider->ReleasePin(provider, STM32F4_GpioInternal_GetControllerId(), RUN_APP_PIN);
