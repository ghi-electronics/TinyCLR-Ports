...
const TinyCLR_Api_Info* STM32F4_SdCard_GetApi() {
    for (auto i = 0; i < TOTAL_SDCARD_CONTROLLERS; i++) {
        ...
        sdCardApi[i].Name = "GHIElectronics.TinyCLR.NativeApis.STM32F4.SdCardController";
...
TinyCLR_Result STM32F4_SdCard_ReadSectors(const TinyCLR_Storage_Controller* self, uint64_t sector, size_t& count, uint8_t* data, uint32_t timeout) {
    int32_t index = 0;
...
TinyCLR_Result STM32F4_SdCard_GetSectorMap(const TinyCLR_Storage_Controller* self, const size_t*& sizes, size_t& count, bool& isUniform) {
    ...
    state->sectorSizes[0] = STM32F4_SD_SECTOR_SIZE;
