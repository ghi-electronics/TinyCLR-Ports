...
void SDIO_ClockCmd(bool newState) {
    *(uint32_t *)CLKCR_CLKEN_BB = (uint32_t)newState;
}
...
void SDIO_StartSDIOReadWait(bool newState) {
    *(uint32_t *)DCTRL_RWSTART_BB = (uint32_t)newState;
}
...
void SDIO_StopSDIOReadWait(bool newState) {
    *(uint32_t *)DCTRL_RWSTOP_BB = (uint32_t)newState;
}
...
void SDIO_SetSDIOReadWaitMode(uint32_t SDIO_ReadWaitMode) {
    *(uint32_t *)DCTRL_RWMOD_BB = SDIO_ReadWaitMode;
}
...
void SDIO_SetSDIOOperation(bool newState) {
    *(uint32_t *)DCTRL_SDIOEN_BB = (uint32_t)newState;
}
...
void SDIO_SendSDIOSuspendCmd(bool newState) {
    *(uint32_t *)CMD_SDIOSUSPEND_BB = (uint32_t)newState;
}
...
void SDIO_CommandCompletionCmd(bool newState) {
    *(uint32_t *)CMD_ENCMDCOMPL_BB = (uint32_t)newState;
}
...
void SDIO_CEATAITCmd(bool newState) {
    *(uint32_t *)CMD_NIEN_BB = (uint32_t)((~((uint32_t)newState)) & ((uint32_t)0x1));
}
...
void SDIO_SendCEATACmd(bool newState) {
    *(uint32_t *)CMD_ATACMD_BB = (uint32_t)newState;
}
...
bool SDIO_GetFlagStatus(uint32_t SDIO_FLAG) {
    ...
    if ((SDIO->STA & SDIO_FLAG) != (uint32_t)false) {
        bitstatus = true;
    }
    else {
        bitstatus = false;
...
bool SDIO_GetITStatus(uint32_t SDIO_IT) {
    ...
    if ((SDIO->STA & SDIO_IT) != (uint32_t)false) {
        bitstatus = true;
    }
    else {
        bitstatus = false;
...
void STM32F4_SdCard_AddApi(const TinyCLR_Api_Manager* apiManager) {
    for (auto i = 0; i < TOTAL_SDCARD_CONTROLLERS; i++) {
        ...
        sdCardControllers[i].Open = &STM32F4_SdCard_Open;
        sdCardControllers[i].Close = &STM32F4_SdCard_Close;
        sdCardControllers[i].Write = &STM32F4_SdCard_Write;
        sdCardControllers[i].Read = &STM32F4_SdCard_Read;
        sdCardControllers[i].Erase = &STM32F4_SdCard_Erases;
        sdCardControllers[i].IsErased = &STM32F4_SdCard_IsErased;
        sdCardControllers[i].GetDescriptor = &STM32F4_SdCard_GetDescriptor;
...
TinyCLR_Result STM32F4_SdCard_Acquire(const TinyCLR_Storage_Controller* self) {
    ...
    if (state->initializeCount == 0) {
        ...
        if (!STM32F4_GpioInternal_OpenMultiPins(sdCardPins[controllerIndex], 6))
            return TinyCLR_Result::SharingViolation;

        for (auto i = 0; i < 6; i++) {
            STM32F4_GpioInternal_ConfigurePin(sdCardPins[controllerIndex][i].number, STM32F4_Gpio_PortMode::AlternateFunction, STM32F4_Gpio_OutputType::PushPull, STM32F4_Gpio_OutputSpeed::High, sdCardPins[controllerIndex][i].number != sdCardPins[controllerIndex][SDCARD_CLK_PIN].number ? STM32F4_Gpio_PullDirection::PullUp : STM32F4_Gpio_PullDirection::None, sdCardPins[controllerIndex][i].alternateFunction);
        }
...
TinyCLR_Result STM32F4_SdCard_GetDescriptor(const TinyCLR_Storage_Controller* self, const TinyCLR_Storage_Descriptor*& descriptor) {
    ...
    state->regionSizes[0] = STM32F4_SD_SECTOR_SIZE;
    state->descriptor.RegionCount = SDCardInfo.CardCapacity / STM32F4_SD_SECTOR_SIZE;
