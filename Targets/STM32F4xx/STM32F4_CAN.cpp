...
void STM32F4_Can_EventCallback(const TinyCLR_Task_Manager* self, const TinyCLR_Api_Manager* apiManager, TinyCLR_Task_Reference task, void* arg);
bool STM32F4_Can_CanPostEvent(int8_t controllerIndex);
...
void STM32F4_Can_AddApi(const TinyCLR_Api_Manager* apiManager) {
    for (int32_t i = 0; i < TOTAL_CAN_CONTROLLERS; i++) {
        ...
        canControllers[i].GetMessagesToWrite = &STM32F4_Can_GetMessagesToWrite;
...
TinyCLR_Result STM32F4_Can_SetReadBufferSize(const TinyCLR_Can_Controller* self, size_t size) {
    ...
    state->canRxMessagesFifo = (STM32F4_Can_Message*)memoryProvider->Allocate(memoryProvider, state->rxBufferSize * sizeof(STM32F4_Can_Message));
...
void STM32F4_Can_RxInterruptHandler(int32_t controllerIndex) {
    ...
    // timestamp
    t = STM32F4_Time_GetSystemTime(nullptr);
...
TinyCLR_Result STM32F4_Can_Acquire(const TinyCLR_Can_Controller* self) {
    ...
    if (state->initializeCount == 0) {
        ...
        STM32F4_Can_SetReadBufferSize(self, canDefaultBuffersSize[controllerIndex]);
...
TinyCLR_Can_Error STM32F4_Can_GetError(uint32_t error) {
    switch (error) {
    case 1:
...
void STM32F4_Can_EventCallback(const TinyCLR_Task_Manager* self, const TinyCLR_Api_Manager* apiManager, TinyCLR_Task_Reference task, void* arg) {
    ...
    if (task == state->messageReceivedCallbackTaskReference) {
        ...
        if (latestCount > 0 && state->messageReceivedEventHandler != nullptr) {
            state->messageReceivedEventHandler(state->controller, latestCount, STM32F4_Time_GetSystemTime(nullptr));
        }

        state->taskManager->Enqueue(state->taskManager, task, STM32F4_Time_GetProcessorTicksForTime(nullptr, CAN_EVENT_POST_DEBOUNCE_TICKS));
    }
        ...
        if ((latestError != 0) && state->errorEventHandler != nullptr) {
            state->errorEventHandler(state->controller, STM32F4_Can_GetError(latestError), STM32F4_Time_GetSystemTime(nullptr));
        }
        state->taskManager->Enqueue(state->taskManager, task, STM32F4_Time_GetProcessorTicksForTime(nullptr, CAN_EVENT_POST_DEBOUNCE_TICKS));
    }
}
...
TinyCLR_Result STM32F4_Can_SetMessageReceivedHandler(const TinyCLR_Can_Controller* self, TinyCLR_Can_MessageReceivedHandler handler) {
    ...
    if (handler != nullptr) {
        ...
        state->taskManager->Create(state->taskManager, STM32F4_Can_EventCallback, (void*)state, false, state->messageReceivedCallbackTaskReference);
        state->taskManager->Enqueue(state->taskManager, state->messageReceivedCallbackTaskReference, STM32F4_Time_GetProcessorTicksForTime(nullptr, CAN_EVENT_POST_DEBOUNCE_TICKS));
    }
...
TinyCLR_Result STM32F4_Can_SetErrorReceivedHandler(const TinyCLR_Can_Controller* self, TinyCLR_Can_ErrorReceivedHandler handler) {
    ...
    if (handler != nullptr) {
        ...
        state->taskManager = (const TinyCLR_Task_Manager*)apiManager->FindDefault(apiManager, TinyCLR_Api_Type::TaskManager);
        state->taskManager->Create(state->taskManager, STM32F4_Can_EventCallback, (void*)state, false, state->errorCallbackTaskReference);
        state->taskManager->Enqueue(state->taskManager, state->errorCallbackTaskReference, STM32F4_Time_GetProcessorTicksForTime(nullptr, CAN_EVENT_POST_DEBOUNCE_TICKS));
    }
    else {
...
TinyCLR_Result STM32F4_Can_Disable(const TinyCLR_Can_Controller* self) {
    ...
    if (state->enable) {
        ...
        if (controllerIndex == 0) {
            STM32F4_InterruptInternal_Deactivate(CAN1_TX_IRQn);
            STM32F4_InterruptInternal_Deactivate(CAN1_RX0_IRQn);
        }
        else {
            STM32F4_InterruptInternal_Deactivate(CAN2_TX_IRQn);
            STM32F4_InterruptInternal_Deactivate(CAN2_RX0_IRQn);
        }
