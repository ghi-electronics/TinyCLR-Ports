...
struct STM32F7_Interrupt_State {
    uint8_t pin;
    int64_t debounce;
...
static STM32F7_Interrupt_State g_int_state[TOTAL_GPIO_INTERRUPT_PINS];
...
const TinyCLR_Api_Info* STM32F7_Gpio_GetApi() {
    for (int32_t i = 0; i < TOTAL_GPIO_CONTROLLERS; i++) {
        ...
        gpioControllers[i].AcquirePin = &STM32F7_Gpio_AcquirePin;
        gpioControllers[i].ReleasePin = &STM32F7_Gpio_ReleasePin;
        ...
        gpioControllers[i].SetValueChangedHandler = &STM32F7_Gpio_SetValueChangedHandler;
...
void STM32F7_Gpio_ISR(int num)  // 0 <= num <= 15
{
    ...
    STM32F7_Interrupt_State* interruptState = &g_int_state[num];
    ...
    if (interruptState->ISR) {
        if (interruptState->debounce) {   // debounce enabled
            if ((STM32F7_Time_GetTimeForProcessorTicks(nullptr, STM32F7_Time_GetCurrentProcessorTicks(nullptr)) - interruptState->lastDebounceTicks) >= g_debounceTicksPin[interruptState->pin]) {
