...
void STM32F7_UsbDevice_InitializeConfiguration(UsClientState *usClientState) {
    ...
    if (usClientState != nullptr) {
        ...
        usbDeviceControllers[controllerIndex].usClientState = usClientState;

        usbDeviceControllers[controllerIndex].endpointType = 0;
        for (auto i = 0; i < usbDeviceControllers[controllerIndex].usClientState->deviceDescriptor.Configurations->Interfaces->EndpointCount; i++) {
            TinyCLR_UsbClient_EndpointDescriptor  *ep = (TinyCLR_UsbClient_EndpointDescriptor*)&usbDeviceControllers[controllerIndex].usClientState->deviceDescriptor.Configurations->Interfaces->Endpoints[i];
...
bool STM32F7_UsbDevice_Initialize(UsClientState* usClientState) {
...
bool STM32F7_UsbDevice_Uninitialize(UsClientState* usClientState) {
    ...
    if (usClientState != nullptr) {
        STM32F7_UsbDevice_ProtectPins(usClientState->controllerIndex, false);
...
void STM32F7_UsbDevice_ResetEvent(OTG_TypeDef* OTG, UsClientState* usClientState) {
...
void STM32F7_UsbDevice_EndpointRxInterrupt(OTG_TypeDef* OTG, UsClientState* usClientState, uint32_t ep, uint32_t count) {
    uint32_t* pd;
...
void STM32F7_UsbDevice_EndpointInInterrupt(OTG_TypeDef* OTG, UsClientState* usClientState, uint32_t ep) {
...
void STM32F7_UsbDevice_HandleSetup(OTG_TypeDef* OTG, UsClientState* usClientState) {
    ...
    STM32F7_UsbDevice_EndpointInInterrupt(OTG, usClientState, 0);
    ...
    if (result == USB_STATE_CONFIGURATION) {
        for (int32_t ep = 1; ep < usClientState->totalEndpointsCount; ep++) {
            if (usClientState->queues[ep] && usClientState->isTxQueue[ep]) {
                STM32F7_UsbDevice_EndpointInInterrupt(OTG, usClientState, ep);
            }
        }
    }
}

void STM32F7_UsbDevice_EndpointOutInterrupt(OTG_TypeDef* OTG, UsClientState* usClientState, uint32_t ep) {
    ...
    if (ep == 0) { // control endpoint
        ...
        STM32F7_UsbDevice_HandleSetup(OTG, usClientState);
    }
...
void STM32F7_UsbDevice_Interrupt(void* param) {
    ...
    UsClientState* usClientState = usbDeviceControllers[controllerIndex].usClientState;
    ...
    while (intPend & OTG_GINTSTS_RXFLVL) { // RxFifo non empty
        ...
        if (status == OTG_GRXSTSP_PKTSTS_PR // data received
            ...
            STM32F7_UsbDevice_EndpointRxInterrupt(OTG, usClientState, ep, count);
        }
        else {
    ...
    if (intPend & OTG_GINTSTS_OEPINT) { // OUT endpoint
        ...
        while (bits) {
            if (bits & 1)
                STM32F7_UsbDevice_EndpointOutInterrupt(OTG, usClientState, ep);
    ...
    if (intPend & OTG_GINTSTS_USBRST) { // reset
        STM32F7_UsbDevice_ResetEvent(OTG, usClientState);
        OTG->GINTSTS = OTG_GINTSTS_USBRST; // clear interrupt
    }
    else {
        if (intPend & OTG_GINTSTS_USBSUSP) { // suspend
            usbDeviceControllers[controllerIndex].previousDeviceState = usClientState->deviceState;
        ...
        if (intPend & OTG_GINTSTS_WKUPINT) { // wakeup
            ...
            usClientState->deviceState = usbDeviceControllers[controllerIndex].previousDeviceState;
...
bool STM32F7_UsbDevice_StartOutput(UsClientState* usClientState, int32_t ep) {
...
bool STM32F7_UsbDevice_RxEnable(UsClientState* usClientState, int32_t ep) {
