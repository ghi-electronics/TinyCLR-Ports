...
TinyCLR_Result STM32F7_Can_GetReadBufferSize(const TinyCLR_Can_Provider* self, size_t& size) {
    ...
    size = canController[channel].can_rxBufferSize == 0 ? g_STM32F7_Can_defaultBuffersSize[channel] : canController[channel].can_rxBufferSize;
...
TinyCLR_Result STM32F7_Can_SetReadBufferSize(const TinyCLR_Can_Provider* self, size_t size) {
    ...
    if (size > 3) {
        ...
        canController[channel].can_rxBufferSize = g_STM32F7_Can_defaultBuffersSize[channel];
...
void STM32_Can_RxInterruptHandler(int32_t channel) {
    ...
    STM32F7_Can_Message *can_msg = &canController[channel].canRxMessagesFifo[canController[channel].can_rx_in];
...
TinyCLR_Result STM32F7_Can_Acquire(const TinyCLR_Can_Provider* self) {
    ...
    if (!STM32F7_GpioInternal_OpenPin(g_STM32F7_Can_Tx_Pins[channel].number))
        return TinyCLR_Result::SharingViolation;

    if (!STM32F7_GpioInternal_OpenPin(g_STM32F7_Can_Rx_Pins[channel].number))
        return TinyCLR_Result::SharingViolation;

    // set pin as analog
    STM32F7_GpioInternal_ConfigurePin(g_STM32F7_Can_Tx_Pins[channel].number, STM32F7_Gpio_PortMode::AlternateFunction, STM32F7_Gpio_OutputType::PushPull, STM32F7_Gpio_OutputSpeed::High, STM32F7_Gpio_PullDirection::PullUp, g_STM32F7_Can_Tx_Pins[channel].alternateFunction);
    STM32F7_GpioInternal_ConfigurePin(g_STM32F7_Can_Rx_Pins[channel].number, STM32F7_Gpio_PortMode::AlternateFunction, STM32F7_Gpio_OutputType::PushPull, STM32F7_Gpio_OutputSpeed::High, STM32F7_Gpio_PullDirection::PullUp, g_STM32F7_Can_Rx_Pins[channel].alternateFunction);
...
TinyCLR_Result STM32F7_Can_SetBitTiming(const TinyCLR_Can_Provider* self, int32_t propagation, int32_t phase1, int32_t phase2, int32_t baudratePrescaler, int32_t synchronizationJumpWidth, int8_t useMultiBitSampling) {
    ...
    if (canController[channel].canRxMessagesFifo == nullptr)
        canController[channel].canRxMessagesFifo = (STM32F7_Can_Message*)memoryProvider->Allocate(memoryProvider, canController[channel].can_rxBufferSize * sizeof(STM32F7_Can_Message));
