...
STM32F7_Gpio_Pin* STM32F7_Pwm_GetGpioPinForChannel(const TinyCLR_Pwm_Provider* self, int32_t pin);
...
const TinyCLR_Api_Info* STM32F7_Pwm_GetApi() {
    for (int i = 0; i < TOTAL_PWM_CONTROLLER; i++) {
        ...
        pwmProviders[i]->Acquire = &STM32F7_Pwm_Acquire;
        pwmProviders[i]->Release = &STM32F7_Pwm_Release;
        pwmProviders[i]->AcquirePin = &STM32F7_Pwm_AcquirePin;
        pwmProviders[i]->ReleasePin = &STM32F7_Pwm_ReleasePin;
        pwmProviders[i]->EnablePin = &STM32F7_Pwm_EnablePin;
        pwmProviders[i]->DisablePin = &STM32F7_Pwm_DisablePin;
        pwmProviders[i]->SetPulseParameters = &STM32F7_Pwm_SetPulseParameters;
        pwmProviders[i]->SetDesiredFrequency = &STM32F7_Pwm_SetDesiredFrequency;
        pwmProviders[i]->GetMinFrequency = &STM32F7_Pwm_GetMinFrequency;
...
TinyCLR_Result STM32F7_Pwm_SetDesiredFrequency(const TinyCLR_Pwm_Provider* self, double& frequency) {
    ...
    for (int p = 0; p < PWM_PER_CONTROLLER; p++)
        if (g_PwmController[self->Index].gpioPin[p].number != PIN_NONE)
            if (STM32F7_Pwm_SetPulseParameters(self, p, g_PwmController[self->Index].dutyCycle[p], g_PwmController[self->Index].invert[p]) != TinyCLR_Result::Success)
...
TinyCLR_Result STM32F7_Pwm_Acquire(const TinyCLR_Pwm_Provider* self) {
    ...
    STM32F7_Pwm_ResetController(self->Index);
