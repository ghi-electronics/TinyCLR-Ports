...
void AT91SAM9Rx64_Power_EnsureTableInitialized() {
    for (auto i = 0; i < TOTAL_POWER_CONTROLLERS; i++) {
        if (powerStates[i].tableInitialized)
            continue;

        powerControllers[i].ApiInfo = &powerApi[i];
        powerControllers[i].Initialize = &AT91SAM9Rx64_Power_Initialize;
        powerControllers[i].Uninitialize = &AT91SAM9Rx64_Power_Uninitialize;
        powerControllers[i].Reset = &AT91SAM9Rx64_Power_Reset;
        powerControllers[i].SetLevel = &AT91SAM9Rx64_Power_SetLevel;
...
const TinyCLR_Api_Info* AT91SAM9Rx64_Power_GetRequiredApi() {
    AT91SAM9Rx64_Power_EnsureTableInitialized();
...
#define SCK_CR_REG      (*(volatile uint32_t*)0xFFFFFD50)
#define SHDW_CR_REG     (*(volatile uint32_t*)AT91C_BASE_SHDWC + 0x00)
#define SHDW_MR_REG     (*(volatile uint32_t*)AT91C_BASE_SHDWC + 0x04)
#define SHDW_SR_REG     (*(volatile uint32_t*)AT91C_BASE_SHDWC + 0x08)
...
void __section(".SectionForInternalRam.RestoreClock") AT91SAM9Rx64_Power_RestoreClock() {
    uint32_t tmp;
...
void __section(".SectionForInternalRam.EnterRcSlowClockMode") AT91SAM9Rx64_Power_EnterRcSlowClockMode(uint32_t dwPres) {
    uint32_t tmp;
    AT91SAM9Rx64_PMC &pmc = AT91::PMC();
...
void __section(".SectionForInternalRam.EnableRcSlowClockMode") AT91SAM9Rx64_Power_EnableRcSlowClockMode() {
...
void __section(".SectionForInternalRam.EnterLowPowerMode") AT91SAM9Rx64_Power_EnterLowPowerMode() {
    volatile uint32_t reg = 0;

    int i;
    AT91SAM9Rx64_DDRS *ddrs = (AT91SAM9Rx64_DDRS *)(AT91C_BASE_SDRAMC);
    ...
    AT91SAM9Rx64_Display_Disable(nullptr);
#endif
    TinyCLR_UsbClient_Uninitialize(nullptr);

    ddrs->DDRSDRC_LPR |= (2 << 12) | 1;  // Delay 120 cycles before enter self refresh mode

    AT91SAM9Rx64_Power_EnableRcSlowClockMode();

    AT91SAM9Rx64_Power_EnterRcSlowClockMode(PMC_MCKR_PRES_CLOCK_DIV64);
...
#ifdef INCLUDE_DISPLAY
    AT91SAM9Rx64_Display_Enable(nullptr);
#endif
}
TinyCLR_Result AT91SAM9Rx64_Power_SetLevel(const TinyCLR_Power_Controller* self, TinyCLR_Power_Level level, TinyCLR_Power_WakeSource wakeSource, uint64_t data) {
    ...
    case TinyCLR_Power_Level::Sleep3: // Sleep
        ...
        AT91SAM9Rx64_Power_EnterLowPowerMode();
