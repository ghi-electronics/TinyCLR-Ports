...
const TinyCLR_Api_Info* AT91_Pwm_GetApi() {
    for (int32_t i = 0; i < TOTAL_PWM_CONTROLLERS; i++) {
        ...
        pwmControllers[i].AcquirePin = &AT91_Pwm_AcquirePin;
        pwmControllers[i].ReleasePin = &AT91_Pwm_ReleasePin;
        pwmControllers[i].EnablePin = &AT91_Pwm_EnablePin;
        pwmControllers[i].DisablePin = &AT91_Pwm_DisablePin;
        ...
        pwmControllers[i].GetPinCount = &AT91_Pwm_GetPinCount;
...
TinyCLR_Result AT91_Pwm_EnablePin(const TinyCLR_Pwm_Controller* self, int32_t pin) {
    ...
    AT91_Gpio_ConfigurePin(actualPin, AT91_Gpio_Direction::Input, driver->gpioPin[pin].peripheralSelection, AT91_Gpio_ResistorMode::Inactive);
...
TinyCLR_Result AT91_Pwm_SetPulseParameters(const TinyCLR_Pwm_Controller* self, int32_t pin, double dutyCycle, bool invertPolarity) {
    ...
    *driver->channelModeReg = (volatile unsigned long)(registerDividerFlag | (pulseBeginsOnHighEdge << 9) | (1 << 10));
    *driver->channelUpdateReg = (volatile unsigned long)(convertedPeriod / (divider * 7.5));
    *driver->dutyCycleReg = (volatile unsigned long)(convertedDuration / (divider * 7.5));
...
TinyCLR_Result AT91_Pwm_SetDesiredFrequency(const TinyCLR_Pwm_Controller* self, double& frequency) {
    ...
    for (int p = 0; p < MAX_PWM_PER_CONTROLLER; p++)
        if (driver->gpioPin[p].number != PIN_NONE)
            if (AT91_Pwm_SetPulseParameters(self, p, driver->dutyCycle[p], driver->invert[p]) != TinyCLR_Result::Success)
...
void AT91_Pwm_ResetController(int32_t controllerIndex) {
    auto driver = &pwmDrivers[controllerIndex];

    for (int p = 0; p < MAX_PWM_PER_CONTROLLER; p++) {
        driver->gpioPin[p] = AT91_Pwm_GetPins(controllerIndex, p);

        if (driver->gpioPin[p].number != PIN_NONE) {
            // Reset values
            driver->channelModeReg = PWM_CHANNEL_MODE_REGISTER(controllerIndex);
            driver->dutyCycleReg = PWM_DUTY_REGISTER(controllerIndex);
            driver->channelUpdateReg = PWM_CHANNEL_UPDATE_REGISTER(controllerIndex);
            driver->invert[p] = false;
            driver->frequency = 0.0;
            driver->dutyCycle[p] = 0.0;

            if (driver->isOpened[p] == true) {
                AT91_Pwm_DisablePin(&pwmControllers[controllerIndex], p);
                AT91_Pwm_ReleasePin(&pwmControllers[controllerIndex], p);
            }
