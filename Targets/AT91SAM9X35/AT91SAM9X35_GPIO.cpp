...
static const AT91SAM9X35_Gpio_PinConfiguration gpioPins[] = AT91SAM9X35_GPIO_PINS;
...
const char* GpioApiNames[TOTAL_GPIO_CONTROLLERS] = {
    "GHIElectronics.TinyCLR.NativeApis.AT91SAM9X35.GpioController\\0"
};

void AT91SAM9X35_Gpio_EnsureTableInitialized() {
    for (auto i = 0; i < TOTAL_GPIO_CONTROLLERS; i++) {
        if (gpioStates[i].tableInitialized)
            continue;

        gpioControllers[i].ApiInfo = &gpioApi[i];
        gpioControllers[i].Acquire = &AT91SAM9X35_Gpio_Acquire;
        gpioControllers[i].Release = &AT91SAM9X35_Gpio_Release;
        gpioControllers[i].OpenPin = &AT91SAM9X35_Gpio_OpenPin;
        gpioControllers[i].ClosePin = &AT91SAM9X35_Gpio_ClosePin;
        gpioControllers[i].Read = &AT91SAM9X35_Gpio_Read;
        gpioControllers[i].Write = &AT91SAM9X35_Gpio_Write;
        gpioControllers[i].IsDriveModeSupported = &AT91SAM9X35_Gpio_IsDriveModeSupported;
        gpioControllers[i].GetDriveMode = &AT91SAM9X35_Gpio_GetDriveMode;
        gpioControllers[i].SetDriveMode = &AT91SAM9X35_Gpio_SetDriveMode;
        gpioControllers[i].GetDebounceTimeout = &AT91SAM9X35_Gpio_GetDebounceTimeout;
        gpioControllers[i].SetDebounceTimeout = &AT91SAM9X35_Gpio_SetDebounceTimeout;
        gpioControllers[i].SetPinChangedHandler = &AT91SAM9X35_Gpio_SetPinChangedHandler;
        gpioControllers[i].GetPinCount = &AT91SAM9X35_Gpio_GetPinCount;
...
const TinyCLR_Api_Info* AT91SAM9X35_Gpio_GetRequiredApi() {
    AT91SAM9X35_Gpio_EnsureTableInitialized();
...
void AT91SAM9X35_Gpio_AddApi(const TinyCLR_Api_Manager* apiManager) {
    ...
    apiManager->SetDefaultName(apiManager, TinyCLR_Api_Type::GpioController, AT91SAM9X35_Gpio_GetRequiredApi()->Name);
}
...
void AT91SAM9X35_Gpio_InterruptHandler(void* param) {
    ...
    for (auto port = 0; port < MAX_PORT; port++) {
        ...
        AT91SAM9X35_PIO &pioX = AT91::PIO(port);
        ...
        while (interruptsActive) {
            ...
            AT91SAM9X35_Gpio_Read(interruptState->controller, interruptState->pin, interruptState->currentValue); // read value as soon as possible
            ...
            if (interruptState->handler && ((expectedEdgeInterger & currentEdgeInterger) || (expectedEdgeInterger == 0))) {
                if (interruptState->debounce) {
                    if ((AT91SAM9X35_Time_GetTimeForProcessorTicks(nullptr, AT91SAM9X35_Time_GetCurrentProcessorTicks(nullptr)) - interruptState->lastDebounceTicks) >= gpioDebounceInTicks[interruptState->pin]) {
                        executeIsr = true;
                    }
                ...
                if (executeIsr)
                    interruptState->handler(interruptState->controller, interruptState->pin, edge, AT91SAM9X35_Time_GetCurrentProcessorTime());
            }
...
TinyCLR_Result AT91SAM9X35_Gpio_SetPinChangedHandler(const TinyCLR_Gpio_Controller* self, uint32_t pin, TinyCLR_Gpio_PinChangeEdge edge, TinyCLR_Gpio_PinChangedHandler handler) {
    ...
    AT91SAM9X35_GpioInternal_EnableInputPin(pin, pinDriveMode[pin]);

    if (handler) {
        interruptState->controller = &gpioControllers[controllerIndex];
        interruptState->pin = (uint8_t)pin;
        interruptState->debounce = AT91SAM9X35_Gpio_GetDebounceTimeout(self, pin);
...
bool AT91SAM9X35_Gpio_Disable_Interrupt(uint32_t pin) {
    return true;
}
...
bool AT91SAM9X35_GpioInternal_ClosePin(int32_t pin) {
    ...
    return AT91SAM9X35_GpioInternal_ConfigurePin(pin, AT91SAM9X35_Gpio_Direction::Input, AT91SAM9X35_Gpio_PeripheralSelection::None, AT91SAM9X35_Gpio_ResistorMode::Inactive);
}
...
bool AT91SAM9X35_GpioInternal_ConfigurePin(int32_t pin, AT91SAM9X35_Gpio_Direction pinDir, AT91SAM9X35_Gpio_PeripheralSelection peripheralSelection, AT91SAM9X35_Gpio_ResistorMode resistorMode, AT91SAM9X35_Gpio_MultiDriver multiDrive, AT91SAM9X35_Gpio_Filter filter, AT91SAM9X35_Gpio_FilterSlowClock filterSlowClock, AT91SAM9X35_Gpio_Schmitt schmitt, AT91SAM9X35_Gpio_DriveSpeed driveSpeed) {
    ...
    if (peripheralSelection == AT91SAM9X35_Gpio_PeripheralSelection::None) {
        pioX.PIO_PER = bitmask; // Enable PIO function
        switch (pinDir) {
        case AT91SAM9X35_Gpio_Direction::Output:
        ...
        case AT91SAM9X35_Gpio_Direction::Input:
            pioX.PIO_ODR = bitmask; // Disable Output

            if (filter == AT91SAM9X35_Gpio_Filter::Enable) {
            ...
            if (filterSlowClock == AT91SAM9X35_Gpio_FilterSlowClock::Enable) {
            ...
            switch (resistorMode) {
            case AT91SAM9X35_Gpio_ResistorMode::Inactive:
                pioX.PIO_PUDR = bitmask;	// Disable the pull up resistor
                PIO_PPDDR(port) = bitmask;	// Disable the pull-down resistor
                break;

            case AT91SAM9X35_Gpio_ResistorMode::PullUp:
                PIO_PPDDR(port) = bitmask;	// Disable the pull-down resistor first.
                pioX.PIO_PUER = bitmask;	// Enable the pull up resistor
                break;

            case AT91SAM9X35_Gpio_ResistorMode::PullDown:
        ...
        if (resistorMode == AT91SAM9X35_Gpio_ResistorMode::Inactive)
            pioX.PIO_PUDR = bitmask;         // Disable the pull up resistor
        else if (resistorMode == AT91SAM9X35_Gpio_ResistorMode::PullUp)
            pioX.PIO_PUER = bitmask;            // Enable the pull up resistor

        switch (peripheralSelection) {
        case AT91SAM9X35_Gpio_PeripheralSelection::PeripheralA:
        ...
        case AT91SAM9X35_Gpio_PeripheralSelection::PeripheralB:
            pioX.PIO_PDR = bitmask;
            pioX.PIO_ABCDSR[0] |= bitmask;
            pioX.PIO_ABCDSR[1] &= ~bitmask;
            break;

        case AT91SAM9X35_Gpio_PeripheralSelection::PeripheralC:
            pioX.PIO_PDR = bitmask;
            pioX.PIO_ABCDSR[0] &= ~bitmask;
            pioX.PIO_ABCDSR[1] |= bitmask;
            break;

        case AT91SAM9X35_Gpio_PeripheralSelection::PeripheralD:
            pioX.PIO_PDR = bitmask;
            pioX.PIO_ABCDSR[0] |= bitmask;
            pioX.PIO_ABCDSR[1] |= bitmask;
            break;
...
bool AT91SAM9X35_GpioInternal_ConfigurePin(int32_t pin, AT91SAM9X35_Gpio_Direction pinDir, AT91SAM9X35_Gpio_PeripheralSelection peripheralSelection, AT91SAM9X35_Gpio_ResistorMode resistorMode) {
    return AT91SAM9X35_GpioInternal_ConfigurePin(pin, pinDir, peripheralSelection, resistorMode, AT91SAM9X35_Gpio_MultiDriver::Disable, AT91SAM9X35_Gpio_Filter::Enable, AT91SAM9X35_Gpio_FilterSlowClock::Disable, AT91SAM9X35_Gpio_Schmitt::Disable, AT91SAM9X35_Gpio_DriveSpeed::Reserved);
}

void AT91SAM9X35_GpioInternal_EnableOutputPin(int32_t pin, bool initialState) {
    AT91SAM9X35_GpioInternal_ConfigurePin(pin, AT91SAM9X35_Gpio_Direction::Output, AT91SAM9X35_Gpio_PeripheralSelection::None, AT91SAM9X35_Gpio_ResistorMode::Inactive);
    AT91SAM9X35_GpioInternal_WritePin(pin, initialState);
}

void AT91SAM9X35_GpioInternal_EnableInputPin(int32_t pin, TinyCLR_Gpio_PinDriveMode mode) {
    switch (mode) {
    case TinyCLR_Gpio_PinDriveMode::Input:
        AT91SAM9X35_GpioInternal_ConfigurePin(pin, AT91SAM9X35_Gpio_Direction::Input, AT91SAM9X35_Gpio_PeripheralSelection::None, AT91SAM9X35_Gpio_ResistorMode::Inactive);
        break;
    case TinyCLR_Gpio_PinDriveMode::InputPullUp:
        AT91SAM9X35_GpioInternal_ConfigurePin(pin, AT91SAM9X35_Gpio_Direction::Input, AT91SAM9X35_Gpio_PeripheralSelection::None, AT91SAM9X35_Gpio_ResistorMode::PullUp);
        break;
    case TinyCLR_Gpio_PinDriveMode::InputPullDown:
        AT91SAM9X35_GpioInternal_ConfigurePin(pin, AT91SAM9X35_Gpio_Direction::Input, AT91SAM9X35_Gpio_PeripheralSelection::None, AT91SAM9X35_Gpio_ResistorMode::PullDown);
        break;
    }
}
...
TinyCLR_Result AT91SAM9X35_Gpio_Read(const TinyCLR_Gpio_Controller* self, uint32_t pin, TinyCLR_Gpio_PinValue& value) {
    ...
    value = AT91SAM9X35_GpioInternal_ReadPin(pin) ? TinyCLR_Gpio_PinValue::High : TinyCLR_Gpio_PinValue::Low;
...
TinyCLR_Result AT91SAM9X35_Gpio_Write(const TinyCLR_Gpio_Controller* self, uint32_t pin, TinyCLR_Gpio_PinValue value) {
    ...
    AT91SAM9X35_GpioInternal_WritePin(pin, value == TinyCLR_Gpio_PinValue::High ? true : false);
...
TinyCLR_Result AT91SAM9X35_Gpio_OpenPin(const TinyCLR_Gpio_Controller* self, uint32_t pin) {
    ...
    if (!AT91SAM9X35_GpioInternal_OpenPin(pin))
...
TinyCLR_Result AT91SAM9X35_Gpio_ClosePin(const TinyCLR_Gpio_Controller* self, uint32_t pin) {
    ...
    AT91SAM9X35_GpioInternal_ClosePin(pin);
...
TinyCLR_Result AT91SAM9X35_Gpio_SetDriveMode(const TinyCLR_Gpio_Controller* self, uint32_t pin, TinyCLR_Gpio_PinDriveMode driveMode) {
    if (pin >= TOTAL_GPIO_PINS || pin < 0)
        return TinyCLR_Result::ArgumentOutOfRange;
    ...
    case TinyCLR_Gpio_PinDriveMode::Output:
        ...
        AT91SAM9X35_Gpio_Write(self, pin, previousOutputValue[pin]);
        break;
...
void AT91SAM9X35_Gpio_Reset() {
    ...
    for (auto pin = 0; pin < AT91SAM9X35_Gpio_GetPinCount(nullptr); pin++) {
        auto& p = gpioPins[pin];

        pinReserved[pin] = false;
        previousOutputValue[pin] = TinyCLR_Gpio_PinValue::Low;

        AT91SAM9X35_Gpio_SetDebounceTimeout(nullptr, pin, DEBOUNCE_DEFAULT_TICKS);

        if (p.apply) {
            AT91SAM9X35_GpioInternal_ConfigurePin(pin, p.direction, p.peripheralSelection, p.resistorMode);

            if (p.direction == AT91SAM9X35_Gpio_Direction::Output)
                AT91SAM9X35_GpioInternal_WritePin(pin, p.outputDirection);
        }
    }

    AT91SAM9X35_InterruptInternal_Activate(AT91C_ID_PIOA_PIOB, (uint32_t*)&AT91SAM9X35_Gpio_InterruptHandler, nullptr);
    AT91SAM9X35_InterruptInternal_Activate(AT91C_ID_PIOC_PIOD, (uint32_t*)&AT91SAM9X35_Gpio_InterruptHandler, nullptr);
