...
void AT91SAM9X35_Uart_SetErrorEvent(int32_t controllerIndex, TinyCLR_Uart_Error error) {
    ...
    if (state->errorEventHandler != nullptr)
        state->errorEventHandler(state->controller, error, AT91SAM9X35_Time_GetCurrentProcessorTime());
}
        ...
        if (state->dataReceivedEventHandler != nullptr) {
            if (canPostEvent) {
                if (state->rxBufferCount > state->lastEventRxBufferCount) {
                    ...
                    state->dataReceivedEventHandler(state->controller, state->rxBufferCount - state->lastEventRxBufferCount, AT91SAM9X35_Time_GetCurrentProcessorTime());
                }
                else {
                    // if user use poll to read data and rxBufferCount <= lastEventRxBufferCount, driver send at least 1 byte comming
                    state->dataReceivedEventHandler(state->controller, 1, AT91SAM9X35_Time_GetCurrentProcessorTime());
                }
