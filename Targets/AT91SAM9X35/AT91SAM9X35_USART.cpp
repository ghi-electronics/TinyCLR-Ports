...
void AT91SAM9X35_Uart_SetErrorEvent(int32_t controllerIndex, TinyCLR_Uart_Error error) {
    ...
    if (state->errorEventHandler != nullptr)
        state->errorEventHandler(state->controller, error, AT91SAM9X35_Time_GetSystemTime());
}
        ...
        if (state->dataReceivedEventHandler != nullptr) {
            if (canPostEvent) {
                if (state->rxBufferCount > state->lastEventRxBufferCount) {
                    ...
                    state->dataReceivedEventHandler(state->controller, state->rxBufferCount - state->lastEventRxBufferCount, AT91SAM9X35_Time_GetSystemTime());
                }
                else {
                    // if user use poll to read data and rxBufferCount <= lastEventRxBufferCount, driver send at least 1 byte comming
                    state->dataReceivedEventHandler(state->controller, 1, AT91SAM9X35_Time_GetSystemTime());
                }
    ...
    if (state->handshaking) {
        ...
        if (sr & AT91SAM9X35_USART::US_CTSIC) {            
            if (state->cleartosendEventHandler != nullptr)
                state->cleartosendEventHandler(state->controller, ctsState, AT91SAM9X35_Time_GetSystemTime());
