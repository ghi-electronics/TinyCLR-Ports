...
int32_t AT91_Uart_GetPeripheralId(int32_t controller) {
    ...
    if (controller == 0) {
        ...
        usartId = (AT91C_ID_USART0 + (controller - 1));
    }
    else {
        usartId = (AT91C_ID_UART0 + (controller - 4));
    }
...
TinyCLR_Result AT91_Uart_SetActiveSettings(const TinyCLR_Uart_Controller* self, int32_t controller, uint32_t baudRate, uint32_t dataBits, TinyCLR_Uart_Parity parity, TinyCLR_Uart_StopBitCount stopBits, TinyCLR_Uart_Handshake handshaking) {
    ...
    AT91_Interrupt_Activate(uartId, (uint32_t*)&AT91_Uart_InterruptHandler, (void*)&g_UartController[controller].controller);
