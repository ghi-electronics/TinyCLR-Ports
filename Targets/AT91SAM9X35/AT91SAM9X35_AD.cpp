...
static const AT91SAM9X35_Gpio_Pin adcPins[] = AT91SAM9X35_ADC_PINS;

const char* adcApiNames[TOTAL_ADC_CONTROLLERS] = {
    "GHIElectronics.TinyCLR.NativeApis.AT91SAM9X35.AdcController\\0"
};
...
void AT91SAM9X35_Adc_AddApi(const TinyCLR_Api_Manager* apiManager) {
    for (int32_t i = 0; i < TOTAL_ADC_CONTROLLERS; i++) {
        ...
        adcControllers[i].Acquire = &AT91SAM9X35_Adc_Acquire;
        adcControllers[i].Release = &AT91SAM9X35_Adc_Release;
        adcControllers[i].OpenChannel = &AT91SAM9X35_Adc_OpenChannel;
        adcControllers[i].CloseChannel = &AT91SAM9X35_Adc_CloseChannel;
        adcControllers[i].ReadChannel = &AT91SAM9X35_Adc_ReadChannel;
        adcControllers[i].SetChannelMode = &AT91SAM9X35_Adc_SetChannelMode;
        adcControllers[i].GetChannelMode = &AT91SAM9X35_Adc_GetChannelMode;
        adcControllers[i].IsChannelModeSupported = &AT91SAM9X35_Adc_IsChannelModeSupported;
        adcControllers[i].GetMinValue = &AT91SAM9X35_Adc_GetMinValue;
        adcControllers[i].GetMaxValue = &AT91SAM9X35_Adc_GetMaxValue;
        adcControllers[i].GetResolutionInBits = &AT91SAM9X35_Adc_GetResolutionInBits;
        adcControllers[i].GetChannelCount = &AT91SAM9X35_Adc_GetChannelCount;
...
int32_t AT91SAM9X35_Adc_GetPinForChannel(int32_t channel) {
    if ((uint32_t)channel >= AT91SAM9X35_Adc_GetChannelCount(&adcControllers[0]))
        return PIN_NONE;

    return AT91SAM9X35_Adc_GetPin(channel);
}

TinyCLR_Result AT91SAM9X35_Adc_OpenChannel(const TinyCLR_Adc_Controller *self, uint32_t channel) {
    if (channel >= AT91SAM9X35_Adc_GetChannelCount(self))
        return TinyCLR_Result::ArgumentOutOfRange;

    if (AT91SAM9X35_Adc_GetPin(channel) == PIN_NONE)
        return TinyCLR_Result::ArgumentInvalid;

    if (channel >= 0 && channel <= AT91SAM9X35_Adc_GetChannelCount(self) && AT91SAM9X35_GpioInternal_OpenPin(AT91SAM9X35_Adc_GetPin(channel))) {
        AT91SAM9X35_GpioInternal_ConfigurePin(AT91SAM9X35_Adc_GetPin(channel), AT91SAM9X35_Gpio_Direction::Input, AT91SAM9X35_Adc_GetPeripheralSelection(channel), AT91SAM9X35_Gpio_ResistorMode::Inactive);
    }
    else {
...
TinyCLR_Result AT91SAM9X35_Adc_CloseChannel(const TinyCLR_Adc_Controller *self, uint32_t channel) {
    ...
    if (state->isOpened[channel])
        AT91SAM9X35_GpioInternal_ClosePin(AT91SAM9X35_Adc_GetPin(channel));
...
int32_t AT91SAM9X35_Adc_AverageMiddleThreeValues(uint32_t *arrayToSort) {
...
void AT91SAM9X35_Adc_SwapArrayValues_Ascending(uint32_t *arrayToSort, int32_t smallNumberArrayPosition, int32_t largeNumberArrayPosition) {
...
void AT91SAM9X35_Adc_SortArray(uint32_t *arrayToSort) {
    ...
    while (!isArraySorted) {
        ...
        for (int32_t x = 0; x < (MAX_AVERAGE_AMOUNT - 1); x++) {
            if (arrayToSort[x] < arrayToSort[x + 1])
                AT91SAM9X35_Adc_SwapArrayValues_Ascending(arrayToSort, x, (x + 1));
            else
                arrayCount++;
        }
...
TinyCLR_Result AT91SAM9X35_Adc_ReadChannel(const TinyCLR_Adc_Controller *self, uint32_t channel, int32_t &value) {
    ...
    for (auto i = 0; i < MAX_AVERAGE_AMOUNT; i++) {
        AT91SAM9X35_Time_Delay(nullptr, 5);
    ...
    AT91SAM9X35_Adc_SortArray(arrayValuesToSort);

    value = AT91SAM9X35_Adc_AverageMiddleThreeValues(arrayValuesToSort);
...
int32_t AT91SAM9X35_Adc_GetMaxValue(const TinyCLR_Adc_Controller *self) {
    return (1 << AT91SAM9X35_Adc_GetResolutionInBits(self)) - 1;
}
...
void AT91SAM9X35_Adc_Reset() {
    for (auto c = 0; c < TOTAL_ADC_CONTROLLERS; c++) {
        for (uint32_t ch = 0; ch < AT91SAM9X35_Adc_GetChannelCount(&adcControllers[c]); ch++) {
            AT91SAM9X35_Adc_CloseChannel(&adcControllers[c], ch);
