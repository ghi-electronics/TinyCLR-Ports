...
struct At91TimerDriver {
    ...
    static void EnableCompareInterrupt(uint32_t Timer) {
        ...
        AT91SAM9X35_TC &TC = AT91::TIMER(Timer);

        (void)TC.TC_SR;
        TC.TC_IER = AT91SAM9X35_TC::TC_CPCS;
        TC.TC_CCR = (AT91SAM9X35_TC::TC_SWTRG | AT91SAM9X35_TC::TC_CLKEN);
    }
    ...
    static void DisableCompareInterrupt(uint32_t Timer) {
        ...
        TC.TC_IDR = AT91SAM9X35_TC::TC_CPCS;
    }
    ...
    static void ForceInterrupt(uint32_t Timer) {
        ...
        AT91SAM9X35_Interrupt_ForceInterrupt(AT91C_ID_TC0_TC1);
    }
    ...
    static bool DidTimerOverFlow(uint32_t Timer) {
        ...
        return (TC.TC_SR & AT91SAM9X35_TC::TC_COVFS) != 0;
    }
...
private:
    struct Descriptors {
        AT91SAM9X35_Interrupt_Callback isr;
        void* arg;
        bool configured;
    };
...
bool At91TimerDriver::Initialize(uint32_t timer, bool freeRunning, uint32_t clkSource, uint32_t* ISR, void* ISR_Param) {
    ...
    if (ISR) {
        if (!AT91SAM9X35_InterruptInternal_Activate(AT91C_ID_TC0_TC1, (uint32_t*)&ISR_TIMER, (void*)timer))
            return false;
    }

    {
        ...
        tc.TC_CCR = AT91SAM9X35_TC::TC_CLKDIS;
        ...
        if (freeRunning == true)
            ...
            tc.TC_CMR = (AT91SAM9X35_TC::TC_CPCTRG | clkSource);

        // Enable Interrupt (Compare RC)
        tc.TC_IER = AT91SAM9X35_TC::TC_CPCS;

        // Enable the clock
        tc.TC_CCR = (AT91SAM9X35_TC::TC_CLKEN | AT91SAM9X35_TC::TC_SWTRG);
    }
...
const char* timeApiNames[TOTAL_TIME_CONTROLLERS] = {
    "GHIElectronics.TinyCLR.NativeApis.AT91SAM9X35.NativeTimeController\\0",

};

void AT91SAM9X35_Time_EnsureTableInitialized() {
    for (auto i = 0; i < TOTAL_TIME_CONTROLLERS; i++) {
        if (timeStates[i].tableInitialized)
            continue;

        timeControllers[i].ApiInfo = &timeApi[i];
        timeControllers[i].Initialize = &AT91SAM9X35_Time_Initialize;
        timeControllers[i].Uninitialize = &AT91SAM9X35_Time_Uninitialize;
        timeControllers[i].GetNativeTime = &AT91SAM9X35_Time_GetCurrentProcessorTicks;
        timeControllers[i].ConvertNativeTimeToSystemTime = &AT91SAM9X35_Time_GetTimeForProcessorTicks;
        timeControllers[i].ConvertSystemTimeToNativeTime = &AT91SAM9X35_Time_GetProcessorTicksForTime;
        timeControllers[i].SetCallback = &AT91SAM9X35_Time_SetTickCallback;
        timeControllers[i].ScheduleCallback = &AT91SAM9X35_Time_SetNextTickCallbackTime;
        timeControllers[i].Wait = &AT91SAM9X35_Time_DelayNative;
...
const TinyCLR_Api_Info* AT91SAM9X35_Time_GetRequiredApi() {
    AT91SAM9X35_Time_EnsureTableInitialized();
...
void AT91SAM9X35_Time_InterruptHandler(void* Param) {
    ...
    if (AT91SAM9X35_Time_GetCurrentProcessorTicks(self) >= state->m_nextCompare) {
        ...
        //
        AT91SAM9X35_Time_SetNextTickCallbackTime(self, state->m_nextCompare);
    }
}

uint32_t AT91SAM9X35_Time_GetTicksPerSecond(const TinyCLR_NativeTime_Controller* self) {
...
uint64_t AT91SAM9X35_Time_GetProcessorTicksForTime(const TinyCLR_NativeTime_Controller* self, uint64_t time) {
    return AT91SAM9X35_Time_MicrosecondsToTicks(self, time / 10);
}

uint64_t AT91SAM9X35_Time_GetCurrentProcessorTime() {
    return AT91SAM9X35_Time_GetTimeForProcessorTicks(nullptr, AT91SAM9X35_Time_GetCurrentProcessorTicks(nullptr));
}
...
uint64_t AT91SAM9X35_Time_GetCurrentProcessorTicks(const TinyCLR_NativeTime_Controller* self) {
    int32_t timer = AT91SAM9X35_TIME_DEFAULT_CONTROLLER_ID;
...
TinyCLR_Result AT91SAM9X35_Time_SetNextTickCallbackTime(const TinyCLR_NativeTime_Controller* self, uint64_t processorTicks) {
    ...
    uint64_t ticks = AT91SAM9X35_Time_GetCurrentProcessorTicks(self);
    ...
    if (state->m_nextCompare >= TIMER_IDLE_VALUE && ticks >= TIMER_IDLE_VALUE) {
        ...
        // Reset current tick
        state->m_lastRead = 0;
        ticks = AT91SAM9X35_Time_GetCurrentProcessorTicks(self);
    }
    ...
    if (ticks >= state->m_nextCompare) {
        ...
        At91TimerDriver::SetCompare(At91TimerDriver::c_SystemTimer,
            (uint32_t)(At91TimerDriver::ReadCounter(At91TimerDriver::c_SystemTimer) + diff));
...
TinyCLR_Result AT91SAM9X35_Time_Initialize(const TinyCLR_NativeTime_Controller* self) {
    ...
    if (!At91TimerDriver::Initialize(At91TimerDriver::c_SystemTimer, true, AT91SAM9X35_TC::TC_CLKS_TIMER_DIV3_CLOCK, (uint32_t*)&AT91SAM9X35_Time_InterruptHandler, (void*)self))
...
void AT91SAM9X35_Time_Delay(const TinyCLR_NativeTime_Controller* self, uint64_t microseconds) {
    ...
    microseconds *= ((AT91SAM9X35_AHB_CLOCK_HZ / 2) / CLOCK_COMMON_FACTOR);
...
void AT91SAM9X35_Time_DelayNative(const TinyCLR_NativeTime_Controller* self, uint64_t nativeTime) {
    ...
    auto microseconds = AT91SAM9X35_Time_GetTimeForProcessorTicks(self, nativeTime) / 10;

    AT91SAM9X35_Time_Delay(self, microseconds);
}
