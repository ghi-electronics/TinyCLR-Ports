...
// usb fifo buffer
static int32_t usb_fifo_buffer_in[LPC24_USB_QUEUE_SIZE];
static int32_t usb_fifo_buffer_out[LPC24_USB_QUEUE_SIZE];
static int32_t usb_fifo_buffer_count[LPC24_USB_QUEUE_SIZE];
...
void LPC24_UsbClient_SetEvent(USB_CONTROLLER_STATE *usbState, uint32_t event) {
...
void LPC24_UsbClient_ClearEvent(USB_CONTROLLER_STATE *usbState, uint32_t event) {
    DISABLE_INTERRUPTS_SCOPED(irq);

    usbState->event &= ~event;
}

void LPC24_UsbClient_ClearQueues(USB_CONTROLLER_STATE *usbState, bool ClrRxQueue, bool ClrTxQueue) {
    DISABLE_INTERRUPTS_SCOPED(irq);

    if (ClrRxQueue) {
        for (int32_t endpoint = 0; endpoint < LPC24_USB_QUEUE_SIZE; endpoint++) {
            ...
            LPC24_UsbClient_RxEnable(usbState, endpoint);
        }
    }
...
void LPC24_UsbClientControllerCallback(USB_CONTROLLER_STATE* usbState) {
    if (usbState->currentState != usbState->deviceState) {
        ...
        if (USB_DEVICE_STATE_CONFIGURED == usbState->currentState) {
            LPC24_UsbClient_ClearQueues(usbState, true, true);
        }
        ...
        case USB_DEVICE_STATE_CONFIGURED:
            ...
            LPC24_UsbClient_ClearQueues(usbState, true, false);
            break;
        }
    }
}

void LPC24_UsbClient_DataCallback(USB_CONTROLLER_STATE* usbState) {
    ...
    if (usbState->expected) {
        usbState->dataCallback = LPC24_UsbClient_DataCallback;
    }
    else {
        usbState->dataCallback = nullptr;
    }
...
uint8_t LPC24_UsbClient_HandleGetStatus(USB_CONTROLLER_STATE* usbState, USB_SETUP_PACKET* Setup) {
    uint16_t* status;
    uint16_t  zero = 0;
...
uint8_t LPC24_UsbClient_HandleClearFeature(USB_CONTROLLER_STATE* usbState, USB_SETUP_PACKET* Setup) {
    TinyCLR_UsbClient_ConfigurationDescriptor * Config;
    uint8_t       retState;
...
uint8_t LPC24_UsbClient_HandleSetFeature(USB_CONTROLLER_STATE* usbState, USB_SETUP_PACKET* Setup) {
    TinyCLR_UsbClient_ConfigurationDescriptor * Config;
    uint8_t       retState;
...
uint8_t LPC24_UsbClient_HandleSetAddress(USB_CONTROLLER_STATE* usbState, USB_SETUP_PACKET* Setup) {
    /* validate setup packet */
    ...
    LPC24_UsbClientControllerCallback(usbState);
...
uint8_t LPC24_UsbClient_HandleConfigurationRequests(USB_CONTROLLER_STATE* usbState, USB_SETUP_PACKET* Setup) {
...
uint8_t LPC24_UsbClient_HandleGetConfiguration(USB_CONTROLLER_STATE* usbState, USB_SETUP_PACKET* Setup) {
    /* validate setup packet */
...
uint8_t LPC24_UsbClient_HandleSetConfiguration(USB_CONTROLLER_STATE* usbState, USB_SETUP_PACKET* Setup, bool DataPhase) {
    /* validate setup packet */
...
uint8_t LPC24_UsbClient_ControlCallback(USB_CONTROLLER_STATE* usbState) {
    ...
    case USB_GET_STATUS:
        return LPC24_UsbClient_HandleGetStatus(usbState, Setup);
    case USB_CLEAR_FEATURE:
        return LPC24_UsbClient_HandleClearFeature(usbState, Setup);
    case USB_SET_FEATURE:
        return LPC24_UsbClient_HandleSetFeature(usbState, Setup);
    case USB_SET_ADDRESS:
        return LPC24_UsbClient_HandleSetAddress(usbState, Setup);
    case USB_GET_CONFIGURATION:
        return LPC24_UsbClient_HandleGetConfiguration(usbState, Setup);
    case USB_SET_CONFIGURATION:
        return LPC24_UsbClient_HandleSetConfiguration(usbState, Setup, true);
    default:
        return LPC24_UsbClient_HandleConfigurationRequests(usbState, Setup);
    }

    return USB_STATE_STALL;
}

USB_PACKET64* LPC24_UsbClient_RxEnqueue(USB_CONTROLLER_STATE* usbState, int32_t endpoint, bool& disableRx) {
    USB_PACKET64* packet;
    ...
    LPC24_UsbClient_SetEvent(usbState, 1 << endpoint);

    return packet;
}

USB_PACKET64* LPC24_UsbClient_TxDequeue(USB_CONTROLLER_STATE* usbState, int32_t endpoint) {
    USB_PACKET64* packet;
...
TinyCLR_Result LPC24_UsbClient_Release(const TinyCLR_UsbClient_Provider* self) {
    ...
    if (usbState->initialized) {
        ...
        LPC24_UsbClient_Uninitialize(usbState);
...
TinyCLR_Result LPC24_UsbClient_Open(const TinyCLR_UsbClient_Provider* self, int32_t& pipe, TinyCLR_UsbClient_PipeMode mode) {
    ...
    for (int32_t i = 0; i < LPC24_USB_QUEUE_SIZE; i++) {
    ...
    while (((uint8_t *)ep) != nullptr && ((uint8_t *)ep) < end) {
        ...
        if (idx > 0) {
            if (apiProvider != nullptr) {
                ...
                usbState->queues[idx] = (USB_PACKET64*)memoryProvider->Allocate(memoryProvider, LPC24_USB_FIFO_BUFFER_SIZE * sizeof(USB_PACKET64));

                if (usbState->queues[idx] == nullptr)
                    return TinyCLR_Result::ArgumentNull;;

                memset(reinterpret_cast<uint8_t*>(usbState->queues[idx]), 0x00, LPC24_USB_FIFO_BUFFER_SIZE * sizeof(USB_PACKET64));
            }
    ...
    if (usbState->currentState == USB_DEVICE_STATE_UNINITIALIZED) {
        LPC24_UsbClient_Initialize(usbState);
    }
...
TinyCLR_Result LPC24_UsbClient_Close(const TinyCLR_UsbClient_Provider* self, int32_t pipe) {
    ...
    if (!usbState->initialized || pipe >= LPC24_USB_QUEUE_SIZE)
        return TinyCLR_Result::NotAvailable;
...
TinyCLR_Result LPC24_UsbClient_Write(const TinyCLR_UsbClient_Provider* self, int32_t pipe, const uint8_t* data, size_t& length) {
    ...
    while (!Done) {
        ...
        if (Packet64 == nullptr) {
            ...
            LPC24_UsbClient_StartOutput(usbState, endpoint);
...
TinyCLR_Result LPC24_UsbClient_Read(const TinyCLR_UsbClient_Provider* self, int32_t pipe, uint8_t* data, size_t& length) {
    ...
    while (count < length) {
        ...
        if (!Packet64) {
            LPC24_UsbClient_ClearEvent(usbState, 1 << endpoint);
            break;
        }
...
LPC24_UsbClientController lcp17_UsbClientController[TOTAL_USB_CONTROLLER];
...
void LPC24_UsbClient_TxPacket(USB_CONTROLLER_STATE* usbState, int32_t endpoint) {
    ...
    for (;;) {
        Packet64 = LPC24_UsbClient_TxDequeue(usbState, endpoint);
...
void LPC24_UsbClient_ProcessEP0(USB_CONTROLLER_STATE *usbState, int32_t in, int32_t setup) {
    ...
    if (setup) {
        ...
        len = LPC24_UsbClient_ReadEP(0x00, lcp17_UsbClientController[usbState->controllerNum].controlPacketBuffer);
        ...
        usbState->ptrData = &lcp17_UsbClientController[usbState->controllerNum].controlPacketBuffer[0];
        usbState->dataSize = len;

        uint8_t result = LPC24_UsbClient_ControlCallback(usbState);
...
void LPC24_UsbClient_Enpoint_RxInterruptHandler(USB_CONTROLLER_STATE *usbState, uint32_t endpoint) {
    bool          DisableRx;
    USB_PACKET64* Packet64 = LPC24_UsbClient_RxEnqueue(usbState, endpoint, DisableRx);
...
void LPC24_UsbClient_ResetEvent(USB_CONTROLLER_STATE *usbState) {
    ...
    // clear all flags
    LPC24_UsbClient_ClearEvent(usbState, 0xFFFFFFFF);
