...
extern USB_PACKET64* LPC24_UsbClient_TxDequeue(USB_CONTROLLER_STATE* State, int queue);
...
bool UsbClient_Driver::OpenPipe(int controller, int32_t& usbPipe, TinyCLR_UsbClient_PipeMode mode) {
    ...
    case TinyCLR_UsbClient_PipeMode::InOut:
        ...
        for (int pipe = 0; pipe < LPC24_USB_QUEUE_SIZE; pipe++) {
        ...
        for (usbPipe = 0; usbPipe < LPC24_USB_QUEUE_SIZE; usbPipe++) {
        ...
        if (usbPipe == LPC24_USB_QUEUE_SIZE)
...
bool UsbClient_Driver::ClosePipe(int controller, int usbPipe) {
    ...
    if (nullptr == State || !State->Initialized || usbPipe >= LPC24_USB_QUEUE_SIZE)
        return false;
...
int UsbClient_Driver::Write(int controller, int usbPipe, const char* Data, size_t size) {
    ...
    if (nullptr == State || usbPipe >= LPC24_USB_QUEUE_SIZE) {
        return -1;
    }
...
bool LPC24_USB_Driver::Initialize(int Controller) {
    ...
    for (auto pipe = 0; pipe < c_Used_Endpoints; pipe++) {
        auto idx = 0;
