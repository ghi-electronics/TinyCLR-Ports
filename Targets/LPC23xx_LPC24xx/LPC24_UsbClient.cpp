...
struct USB_CONTROLLER_STATE {
    ...
    USB_PACKET64                                   	            *Queues[LPC24_USB_QUEUE_SIZE];
    ...
    USB_PIPE_MAP                                                pipes[LPC24_USB_QUEUE_SIZE];
...
void LPC24_UsbClient_ClearEndpoints(int32_t endpoint);
bool LPC24_UsbClient_RxEnable(USB_CONTROLLER_STATE* State, int endpoint);

TinyCLR_UsbClient_DataReceivedHandler LPC24_UsbClient_DataReceivedHandler;
TinyCLR_UsbClient_OsExtendedPropertyHandler LPC24_UsbClient_OsExtendedPropertyHandler;
...
LPC24_UsbClient_State LPC24_UsbClient_ControllerState;
...
void LPC24_UsbClient_SetEvent(USB_CONTROLLER_STATE *State, uint32_t event) {
    ...
    if (old_event != State->Event) {
        LPC24_UsbClient_DataReceivedHandler(nullptr);
    }
}

void LPC24_UsbClient_ClearEvent(USB_CONTROLLER_STATE *State, uint32_t event) {
    DISABLE_INTERRUPTS_SCOPED(irq);

    State->Event &= ~event;
}

void LPC24_UsbClient_ClearQueues(USB_CONTROLLER_STATE *State, bool ClrRxQueue, bool ClrTxQueue) {
    DISABLE_INTERRUPTS_SCOPED(irq);
    ...
    if (ClrRxQueue) {
        for (int endpoint = 0; endpoint < LPC24_USB_QUEUE_SIZE; endpoint++) {
            ...
            LPC24_UsbClient_ClearEndpoints(endpoint);
...
void LPC24_UsbClient_StateCallback(USB_CONTROLLER_STATE* State) {
    if (State->CurrentState != State->DeviceState) {
        ...
        if (USB_DEVICE_STATE_CONFIGURED == State->CurrentState) {
            LPC24_UsbClient_ClearQueues(State, true, true);
        }
        ...
        case USB_DEVICE_STATE_CONFIGURED:
            ...
            LPC24_UsbClient_ClearQueues(State, true, false);
            break;
        }
    }
}

void LPC24_UsbClient_DataCallback(USB_CONTROLLER_STATE* State) {
    ...
    if (State->Expected) {
        State->DataCallback = LPC24_UsbClient_DataCallback;
    }
    else {
        State->DataCallback = nullptr;
    }
...
uint8_t LPC24_UsbClient_HandleGetStatus(USB_CONTROLLER_STATE* State, USB_SETUP_PACKET* Setup) {
    uint16_t* status;
    uint16_t  zero = 0;
...
uint8_t LPC24_UsbClient_HandleClearFeature(USB_CONTROLLER_STATE* State, USB_SETUP_PACKET* Setup) {
    TinyCLR_UsbClient_ConfigurationDescriptor * Config;
    uint8_t       retState;
...
uint8_t LPC24_UsbClient_HandleSetFeature(USB_CONTROLLER_STATE* State, USB_SETUP_PACKET* Setup) {
    TinyCLR_UsbClient_ConfigurationDescriptor * Config;
    uint8_t       retState;
...
uint8_t LPC24_UsbClient_HandleSetAddress(USB_CONTROLLER_STATE* State, USB_SETUP_PACKET* Setup) {
    /* validate setup packet */
...
uint8_t LPC24_UsbClient_HandleConfigurationRequests(USB_CONTROLLER_STATE* State, USB_SETUP_PACKET* Setup) {
...
uint8_t LPC24_UsbClient_HandleGetConfiguration(USB_CONTROLLER_STATE* State, USB_SETUP_PACKET* Setup) {
    /* validate setup packet */
...
uint8_t LPC24_UsbClient_ControlCallback(USB_CONTROLLER_STATE* State) {
    ...
    case USB_GET_STATUS:
        return LPC24_UsbClient_HandleGetStatus(State, Setup);
    case USB_CLEAR_FEATURE:
        return LPC24_UsbClient_HandleClearFeature(State, Setup);
    case USB_SET_FEATURE:
        return LPC24_UsbClient_HandleSetFeature(State, Setup);
    case USB_SET_ADDRESS:
        return LPC24_UsbClient_HandleSetAddress(State, Setup);
    case USB_GET_CONFIGURATION:
        return LPC24_UsbClient_HandleGetConfiguration(State, Setup);
    case USB_SET_CONFIGURATION:
        return LPC24_UsbClient_HandleSetConfiguration(State, Setup, true);
    default:
        return LPC24_UsbClient_HandleConfigurationRequests(State, Setup);
    }
...
USB_PACKET64* LPC24_UsbClient_RxEnqueue(USB_CONTROLLER_STATE* State, int endpoint, bool& disableRx) {
    ...
    if (usb_fifo_buffer_count[endpoint] == LPC24_USB_FIFO_BUFFER_SIZE) {
        disableRx = true;
    ...
    if (usb_fifo_buffer_in[endpoint] == LPC24_USB_FIFO_BUFFER_SIZE)
        usb_fifo_buffer_in[endpoint] = 0;

    LPC24_UsbClient_SetEvent(State, 1 << endpoint);

    return packet;
}

USB_PACKET64* LPC24_UsbClient_TxDequeue(USB_CONTROLLER_STATE* State, int endpoint) {
    USB_PACKET64* packet;
    ...
    if (usb_fifo_buffer_out[endpoint] == LPC24_USB_FIFO_BUFFER_SIZE)
...
const TinyCLR_Api_Info* LPC24_UsbClient_GetApi() {
    ...
    usbClientProvider.SetOsExtendedPropertyHandler = &LPC24_UsbClient_SetOsExtendedPropertyHandler;
...
TinyCLR_Result LPC24_UsbClient_Acquire(const TinyCLR_UsbClient_Provider* self) {
    ...
    LPC24_UsbClient_OsExtendedPropertyHandler(self, osStringDescriptor, xCompatibleOsId, xPropertiesOsWinUsb);

    USB_CONTROLLER_STATE *State = &LPC24_UsbClient_ControllerState.state;
    ...
    State->EndpointCount = USB_MAX_EP_COUNT;
...
TinyCLR_Result LPC24_UsbClient_Open(const TinyCLR_UsbClient_Provider* self, int32_t& pipe, TinyCLR_UsbClient_PipeMode mode) {
    ...
    for (auto i = 0; i < SIZEOF_ARRAY(LPC24_UsbClient_EndpointMap); i++) {
        ...
        if (writeEp == USB_ENDPOINT_NULL && ((LPC24_UsbClient_EndpointMap[i] & ENDPOINT_DIR_IN_MASK) == ENDPOINT_DIR_IN_MASK)) {
            writeEp = i;
        ...
        if (readEp == USB_ENDPOINT_NULL && ((LPC24_UsbClient_EndpointMap[i] & ENDPOINT_DIR_OUT_MASK) == ENDPOINT_DIR_OUT_MASK)) {
            readEp = i;
            LPC24_UsbClient_EndpointMap[i] |= ENDPOINT_INUSED_MASK;
    ...
    if ((readEp == USB_ENDPOINT_NULL && writeEp == USB_ENDPOINT_NULL)
        || (readEp != USB_ENDPOINT_NULL && (readEp < 1 || readEp >= LPC24_USB_QUEUE_SIZE))
        || (writeEp != USB_ENDPOINT_NULL && (writeEp < 1 || writeEp >= LPC24_USB_QUEUE_SIZE)))
        return TinyCLR_Result::NotAvailable;

    // The specified endpoints must not be in use by another pipe
    for (int i = 0; i < LPC24_USB_QUEUE_SIZE; i++) {
    ...
    while (((uint8_t *)ep) != nullptr && ((uint8_t *)ep) < end) {
        ...
        if (idx > 0) {
            ...
            LPC24_UsbClient_ClearEndpoints(idx);
...
TinyCLR_Result LPC24_UsbClient_Close(const TinyCLR_UsbClient_Provider* self, int32_t pipe) {
    ...
    if (!State->Initialized || pipe >= LPC24_USB_QUEUE_SIZE)
        return TinyCLR_Result::NotAvailable;
...
TinyCLR_Result LPC24_UsbClient_Read(const TinyCLR_UsbClient_Provider* self, int32_t pipe, uint8_t* data, size_t& length) {
    ...
    while (count < length) {
        ...
        if (!Packet64) {
            LPC24_UsbClient_ClearEvent(State, 1 << endpoint);
            break;
        }
...
TinyCLR_Result LPC24_UsbClient_Flush(const TinyCLR_UsbClient_Provider* self, int32_t pipe) {
    ...
    while (usb_fifo_buffer_count[endpoint] > 0 && retries > 0) {
        ...
        LPC24_Time_Delay(nullptr, queueCnt == usb_fifo_buffer_count[endpoint] ? 100 : 0); // don't call Events_WaitForEventsXXX because it will turn off interrupts
...
TinyCLR_Result LPC24_UsbClient_SetDataReceivedHandler(const TinyCLR_UsbClient_Provider* self, TinyCLR_UsbClient_DataReceivedHandler handler) {
    ...
    LPC24_UsbClient_DataReceivedHandler = handler;
...
TinyCLR_Result LPC24_UsbClient_SetOsExtendedPropertyHandler(const TinyCLR_UsbClient_Provider* self, TinyCLR_UsbClient_OsExtendedPropertyHandler handler) {
    ...
    LPC24_UsbClient_OsExtendedPropertyHandler = handler;
...
void LPC24_UsbClient_Reset() {
    for (auto pipe = 0; pipe < LPC24_USB_QUEUE_SIZE; pipe++) {
        LPC24_UsbClient_Close(&usbClientProvider, pipe);
    }

    LPC24_UsbClient_Release(&usbClientProvider);
}
...
static EndpointConfiguration EndpointInit[USB_MAX_EP_COUNT];     // Corresponds to endpoint configuration RAM at LPC24xx_USB::UDCCRx
static int nacking_rx_OUT_data[USB_MAX_EP_COUNT];

bool LPC24_UsbClient_ProtectPins(int Controller, bool On);
void LPC24_UsbClient_Global_ISR(void* Param);
void LPC24_UsbClient_TxPacket(USB_CONTROLLER_STATE* State, int endpoint);
void LPC24_UsbClient_ProcessEP0(int in, int setup);
void LPC24_UsbClient_ProcessEndPoint(int ep, int in);
void LPC24_UsbClient_EP_TxISR(uint32_t endpoint);
void LPC24_UsbClient_EP_RxISR(uint32_t endpoint);
void LPC24_UsbClient_ResetEvent();
void LPC24_UsbClient_SuspendEvent();
void LPC24_UsbClient_ResumeEvent();
void LPC24_UsbClient_ControlNext();
...
bool LPC24_UsbClient_Initialize(int controller) {
    ...
    LPC24_Interrupt_Activate(USB_IRQn, (uint32_t*)&LPC24_UsbClient_Global_ISR, 0);

    for (int i = 0; i < USB_MAX_EP_COUNT; i++)
        EndpointInit[i].word = 0;       // All useable endpoints initialize to unused

    for (auto pipe = 0; pipe < USB_MAX_EP_COUNT; pipe++) {
        auto idx = 0;
        if (State->pipes[pipe].RxEP != USB_ENDPOINT_NULL) {
            idx = State->pipes[pipe].RxEP;
            EndpointInit[idx].bits.ED = 0;
            EndpointInit[idx].bits.DE = 0;
        }

        if (State->pipes[pipe].TxEP != USB_ENDPOINT_NULL) {
    ...
    State->EndpointStatus = &LPC24_UsbClient_ControllerState.EndpointStatus[0];
    State->EndpointCount = USB_MAX_EP_COUNT;
    State->PacketSize = USB_MAX_EP0_SIZE;
...
bool LPC24_UsbClient_Uninitialize(int controller) {
    ...
    LPC24_UsbClient_ProtectPins(controller, false);
...
bool LPC24_UsbClient_StartOutput(USB_CONTROLLER_STATE* State, int endpoint) {
    ...
    if (!LPC24_UsbClient_ControllerState.txRunning[endpoint]) {
        LPC24_UsbClient_ControllerState.txRunning[endpoint] = true;

        // Calling both LPC24_UsbClient_TxPacket & EP_TxISR in this routine could cause a TX FIFO overflow
        LPC24_UsbClient_TxPacket(State, endpoint);
    }
        ...
        if ((USBEpIntSt & (1 << n)))//&& (USBEpIntEn & (1 << n)) )//only if enabled
        {
            ...
            if (m == 0)//EP0
            {
                ...
                {
                    LPC24_UsbClient_ProcessEP0(0, 1);// out setup
                }
                else {
                    if ((n & 1) == 0)                /* OUT Endpoint */
                    {
                        LPC24_UsbClient_ProcessEP0(0, 0);// out not setup
                    }
                    else {
                        LPC24_UsbClient_ProcessEP0(1, 0);// in not setup
                    }
                }
            }
            else {
                if (State->Queues[m] && State->IsTxQueue[endpoint])
                    LPC24_UsbClient_ProcessEndPoint(m, 1);//out
                else
                    LPC24_UsbClient_ProcessEndPoint(m, 0);//in
            }
        }
    }
...
bool LPC24_UsbClient_RxEnable(USB_CONTROLLER_STATE* State, int endpoint) {
    if (nullptr == State || endpoint >= USB_MAX_EP_COUNT)
        return false;

    DISABLE_INTERRUPTS_SCOPED(irq);

    if (nacking_rx_OUT_data[endpoint])
        LPC24_UsbClient_EP_RxISR(endpoint);//force interrupt to read the pending EP
...
void LPC24_UsbClient_ProcessEndPoint(int ep, int in) {
    ...
    if (in) {
        LPC24_UsbClient_EP_TxISR(ep);
    }
    else {
        USBEpIntClr = 1 << LPC24_UsbClient_EPAdr(ep, in);
        while ((USBDevIntSt & CDFULL_INT) == 0);
        val = USBCmdData;
        LPC24_UsbClient_EP_RxISR(ep);
    }
}
...
void LPC24_UsbClient_TxPacket(USB_CONTROLLER_STATE* State, int endpoint) {
    ...
    if (Packet64) {
        ...
        LPC24_UsbClient_ControllerState.txNeedZLPS[endpoint] = false;
        if (Packet64->Size == 64)
            LPC24_UsbClient_ControllerState.txNeedZLPS[endpoint] = true;
    }
    else {
        // send the zero length packet since we landed on the FIFO boundary before
        // (and we queued a zero length packet to transmit)
        if (LPC24_UsbClient_ControllerState.txNeedZLPS[endpoint]) {
            USB_WriteEP(endpoint, (uint8_t*)nullptr, 0);
            LPC24_UsbClient_ControllerState.txNeedZLPS[endpoint] = false;
        }

        // no more data
        LPC24_UsbClient_ControllerState.txRunning[endpoint] = false;
    }
}
void LPC24_UsbClient_ControlNext() {
    USB_CONTROLLER_STATE *State = &LPC24_UsbClient_ControllerState.state;
    ...
    if (State->DataCallback) {
        ...
        if (State->DataSize == 0) {
            ...
            if (State->DataSize < USB_MAX_EP0_SIZE) // If packet is less than full length
            {
                State->DataCallback = nullptr; // Stop sending stuff if we're done
            }
            ...
            if (LPC24_UsbClient_ControllerState.firstDescriptorPacket) {
                State->DataCallback = nullptr;
            }
...
void LPC24_UsbClient_Global_ISR(void* Param) {
    ...
    if (disr & DEV_STAT_INT) {
        ...
        if (val & DEV_SUS_CH)                  /* Suspend/Resume */
        {
            ...
            {
                LPC24_UsbClient_SuspendEvent();
            }
            else                               /* Resume */
            {
                LPC24_UsbClient_ResumeEvent();
            }
        }
    ...
    if (disr & EP_SLOW_INT) {
        for (n = 0; n < USB_EP_NUM; n++)     /* Check All Endpoints */
        {
            if ((USBEpIntSt & (1 << n))) {
                ...
                if ((n & 1) == 0)                /* OUT Endpoint */
                {
                    LPC24_UsbClient_ProcessEndPoint(m, 0);//out
                }
                else                           /* IN Endpoint */
                {
                    LPC24_UsbClient_ProcessEndPoint(m, 1);//in
                }
            }
        }
    }
...
void LPC24_UsbClient_ProcessEP0(int in, int setup) {
    ...
    if (setup) {
        ...
        len = LPC24_UsbClient_ReadEP(0x00, LPC24_UsbClient_ControllerState.controlPacketBuffer);

        // special handling for the very first SETUP command - Getdescriptor[DeviceType], the host looks for 8 bytes data only
        USB_SETUP_PACKET* Setup = (USB_SETUP_PACKET*)&LPC24_UsbClient_ControllerState.controlPacketBuffer[0];
        if ((Setup->bRequest == USB_GET_DESCRIPTOR) && (((Setup->wValue & 0xFF00) >> 8) == USB_DEVICE_DESCRIPTOR_TYPE) && (Setup->wLength != 0x12))
            LPC24_UsbClient_ControllerState.firstDescriptorPacket = true;
        else
            LPC24_UsbClient_ControllerState.firstDescriptorPacket = false;

        // send it to the upper layer
        State->Data = &LPC24_UsbClient_ControllerState.controlPacketBuffer[0];
        State->DataSize = len;
        ...
        case USB_STATE_ADDRESS:
            LPC24_UsbClient_DeviceAddress = State->Address | 0x80;
            break;
            ...
            if (result == USB_STATE_CONFIGURATION) {
                for (int ep = 0; ep < USB_MAX_EP_COUNT; ep++) {
                    if (State->IsTxQueue[ep])
                        LPC24_UsbClient_StartOutput(State, ep);
                }
            }
        }
    }
        ...
        LPC24_UsbClient_ControlNext();      // See if there is more to send
...
void LPC24_UsbClient_EP_TxISR(uint32_t endpoint) {
    ...
    if (USBEpIntSt & (1 << LPC24_UsbClient_EPAdr(endpoint, 1)))//done sending?
    {
        ...
        LPC24_UsbClient_TxPacket(&LPC24_UsbClient_ControllerState.state, endpoint);
    }
}

void LPC24_UsbClient_EP_RxISR(uint32_t endpoint) {
    bool          DisableRx;
    USB_PACKET64* Packet64 = LPC24_UsbClient_RxEnqueue(&LPC24_UsbClient_ControllerState.state, endpoint, DisableRx);
...
void LPC24_UsbClient_SuspendEvent() {
    ...
    LPC24_UsbClient_ControllerState.previousDeviceState = State->DeviceState;
...
void LPC24_UsbClient_ResumeEvent() {
    ...
    State->DeviceState = LPC24_UsbClient_ControllerState.previousDeviceState;
...
void LPC24_UsbClient_ResetEvent() {
    ...
    // clear all flags
    LPC24_UsbClient_ClearEvent(State, 0xFFFFFFFF);

    for (int ep = 0; ep < USB_MAX_EP_COUNT; ep++) {
        LPC24_UsbClient_ControllerState.txRunning[ep] = false;
        LPC24_UsbClient_ControllerState.txNeedZLPS[ep] = false;
    }
