...
void LPC24_Can_AddApi(const TinyCLR_Api_Manager* apiManager) {
    for (int32_t i = 0; i < TOTAL_CAN_CONTROLLERS; i++) {
        ...
        canControllers[i].Enable = &LPC24_Can_Enable;
        canControllers[i].Disable = &LPC24_Can_Disable;
        canControllers[i].CanWriteMessage = &LPC24_Can_CanWriteMessage;
        canControllers[i].CanReadMessage = &LPC24_Can_CanReadMessage;
...
TinyCLR_Result LPC24_Can_WriteMessage(const TinyCLR_Can_Controller* self, const TinyCLR_Can_Message* messages, size_t& len) {
...
TinyCLR_Result LPC24_Can_ReadMessage(const TinyCLR_Can_Controller* self, TinyCLR_Can_Message* messages, size_t& len) {
    if (len != 1) return TinyCLR_Result::NotSupported;
...
void LPC24_Can_EventCallback(const TinyCLR_Task_Manager* self, const TinyCLR_Api_Manager* apiManager, TinyCLR_Task_Reference task, void* arg) {
    ...
    if (task == state->messageReceivedCallbackTaskReference) {
        ...
        if (latestCount > 0 && state->messageReceivedEventHandler != nullptr) {
            state->messageReceivedEventHandler(state->controller, latestCount, LPC24_Time_GetSystemTime(nullptr));
        }
        ...
        if ((latestError != 0) && state->errorEventHandler != nullptr) {
            state->errorEventHandler(state->controller, LPC24_Can_GetError(latestError), LPC24_Time_GetSystemTime(nullptr));
        }
        state->taskManager->Enqueue(state->taskManager, task, LPC24_Time_GetProcessorTicksForTime(nullptr, CAN_EVENT_POST_DEBOUNCE_TICKS));
    }
}
...
TinyCLR_Result LPC24_Can_SetMessageReceivedHandler(const TinyCLR_Can_Controller* self, TinyCLR_Can_MessageReceivedHandler handler) {
    ...
    if (handler != nullptr) {
        ...
        state->taskManager->Enqueue(state->taskManager, state->messageReceivedCallbackTaskReference, LPC24_Time_GetProcessorTicksForTime(nullptr, CAN_EVENT_POST_DEBOUNCE_TICKS));
    }
...
TinyCLR_Result LPC24_Can_SetErrorReceivedHandler(const TinyCLR_Can_Controller* self, TinyCLR_Can_ErrorReceivedHandler handler) {
    ...
    if (handler != nullptr) {
        ...
        state->taskManager->Enqueue(state->taskManager, state->errorCallbackTaskReference, LPC24_Time_GetProcessorTicksForTime(nullptr, CAN_EVENT_POST_DEBOUNCE_TICKS));
    }
    else {
