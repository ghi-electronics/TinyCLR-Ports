...
const TinyCLR_Api_Info* LPC24_Can_GetApi() {
    for (int32_t i = 0; i < TOTAL_CAN_CONTROLLERS; i++) {
        ...
        canControllers[i].Release = &LPC24_Can_Release;
        canControllers[i].Reset = &LPC24_Can_SoftReset;
        canControllers[i].WriteMessage = &LPC24_Can_WriteMessage;
        canControllers[i].ReadMessage = &LPC24_Can_ReadMessage;
        canControllers[i].SetBitTiming = &LPC24_Can_SetBitTiming;
        canControllers[i].GetUnreadMessageCount = &LPC24_Can_GetUnreadMessageCount;
        ...
        canControllers[i].IsWritingAllowed = &LPC24_Can_IsWritingAllowed;
        ...
        canControllers[i].GetReadBufferSize = &LPC24_Can_GetReadBufferSize;
        canControllers[i].SetReadBufferSize = &LPC24_Can_SetReadBufferSize;
        canControllers[i].GetWriteBufferSize = &LPC24_Can_GetWriteBufferSize;
...
void CAN_ISR_Rx(int32_t controllerIndex) {
    ...
    LPC24_Can_Message *can_msg = &driver->canRxMessagesFifo[driver->can_rx_in];
...
TinyCLR_Result LPC24_Can_SetBitTiming(const TinyCLR_Can_Controller* self, int32_t propagation, int32_t phase1, int32_t phase2, int32_t baudratePrescaler, int32_t synchronizationJumpWidth, int8_t useMultiBitSampling) {
    ...
    if (driver->canRxMessagesFifo == nullptr)
        driver->canRxMessagesFifo = (LPC24_Can_Message*)memoryProvider->Allocate(memoryProvider, driver->can_rxBufferSize * sizeof(LPC24_Can_Message));
