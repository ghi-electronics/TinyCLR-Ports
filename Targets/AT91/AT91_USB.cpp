...
bool AT91_UsbClient_StartOutput(USB_CONTROLLER_STATE* State, int ep) {
    ...
    return AT91_USB_Driver::StartOutput(State, ep);
}

bool AT91_UsbClient_RxEnable(USB_CONTROLLER_STATE* State, int ep) {

    return AT91_USB_Driver::RxEnable(State, ep);
}
...
static EndpointConfiguration EndpointInit[AT91_USB_Driver::c_Used_Endpoints];     // Corresponds to endpoint configuration RAM at AT91_USB::UDCCRx
...
static int nacking_rx_OUT_data[AT91_USB_Driver::c_Used_Endpoints];
...
void AT91_USB_Driver::ProcessEndPoint(int ep, int in) {
    int val;
...
bool AT91_USB_Driver::Initialize(int Controller) {
    int endpointsUsed = 0;
    ...
    AT91_Interrupt_Activate(USB_IRQn, (uint32_t*)&Global_ISR, 0);
...
bool AT91_USB_Driver::Uninitialize(int Controller) {
    GLOBAL_LOCK(irq);

    ProtectPins(Controller, true);

    g_AT91_USB_Driver.pUsbControllerState = nullptr;

    //AT91_Interrupt_Deactivate( AT91_AITC::c_IRQ_INDEX_USB_CLIENT );
    AT91_Interrupt_Deactivate(USB_IRQn);
...
void AT91_USB_Driver::ClearTxQueue(USB_CONTROLLER_STATE* State, int endpoint) {
    ...
    while (nullptr != AT91_UsbClient_TxDequeue(State, endpoint, true)) {
        State->Queues[endpoint]->erase(State->Queues[endpoint]->begin());
    }
}
...
bool AT91_UsbClient_SoftReset(int controller) {
    AT91_Interrupt_Activate(USB_IRQn, (uint32_t*)&AT91_USB_Driver::Global_ISR, 0);

    return true;
}
//--//

void AT91_USB_Driver::StartHardware() {
    *(uint32_t*)0xE01FC0C4 |= 0x80000000;
    USBClkCtrl = (1 << 1) | (1 << 3) | (1 << 4);

    AT91_UsbClient_PinConfiguration();
    ...
    AT91_Time_Delay(nullptr, 120 * 1000);

    USB_Connect(true);

}

void AT91_USB_Driver::StopHardware() {
    USB_Connect(false);
}
...
void AT91_USB_Driver::TxPacket(USB_CONTROLLER_STATE* State, int endpoint) {
    ...
    for (;;) {
        ...
        Packet64 = AT91_UsbClient_TxDequeue(State, endpoint, true);
...
void AT91_USB_Driver::EP0_ISR(uint32_t Param) {
    while (1);//this is not used?????????
}
void AT91_USB_Driver::ProcessEP0(int in, int setup) {
    uint32_t EP_INTR;
    int i;
    ...
    if (setup)//(EP_INTR & AT91_USB::UDCICR__PCKT) && (USB.UDCCSRx[0] & AT91_USB::UDCCSR__OPC))
    {
        USB_CONTROLLER_STATE *State = g_AT91_USB_Driver.pUsbControllerState;

        uint8_t   len = 0;//USB.UDCBCRx[0] & AT91_USB::UDCBCR_mask;        // Received packet length

        len = USB_ReadEP(0x00, g_AT91_USB_Driver.ControlPacketBuffer);
    ...
    else if (in)//(EP_INTR & AT91_USB::UDCICR__PCKT) && (USB.UDCCSRx[0] & (AT91_USB::UDCCSR__IPR | AT91_USB::UDCCSR__SST)) == 0)
    {
...
void AT91_USB_Driver::EP_TxISR(uint32_t endpoint) {
    ASSERT_IRQ_MUST_BE_OFF();
    ...
    if (USBEpIntSt & (1 << EPAdr(endpoint, 1)))//done sending?
    {
        ...
        TxPacket(g_AT91_USB_Driver.pUsbControllerState, endpoint);
    }
...
void AT91_USB_Driver::EP_RxISR(uint32_t endpoint) {
    bool          DisableRx;
    ...
    USB_PACKET64* Packet64 = AT91_UsbClient_RxEnqueue(g_AT91_USB_Driver.pUsbControllerState, endpoint, DisableRx);
    ...
    if (Packet64) {
        uint8_t   len = 0;//USB.UDCBCRx[EPno] & AT91_USB::UDCBCR_mask;
    ...
    if (State) {
        if (On) {
            ...
            if (g_AT91_USB_Driver.PinsProtected) {
                ...
                //CPU_GPIO_EnableInputPin(AT91_USB::c_USBC_GPION_DET, false, nullptr, GPIO_INT_NONE, RESISTOR_DISABLED);
                //CPU_GPIO_EnableOutputPin(AT91_USB::c_USBC_GPIOX_EN, false);       // Don't signal the host yet
